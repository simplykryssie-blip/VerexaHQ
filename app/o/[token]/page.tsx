...
export default async function PublicOrganizerPage({ params }: { params: { token: string } }) {
  const supabase = createClient();
  const { data } = await supabase.rpc("get_public_organizer_template", { p_token: params.token });
  ...
  return <PublicOrganizerForm token={params.token} data={data as never} />;
}
