...
// Uploads a drawn signature for the standalone public engagement-letter
// link (/e/[token]) before the actual sign_public_engagement_letter[_with_signup]
// RPC call -- there's no logged-in user on this page to gate a storage RLS
// insert policy against, so this runs server-side with the service role
// after validating the token the same way the sign RPCs do.
export async function POST(request: Request, { params }: { params: { token: string } }) {
  const allowed = await checkRateLimit(`e-signature-image:${clientIp(request)}`, 20, 60);
  if (!allowed) {
