...
export default async function ManageCoursePage({ params }: { params: { courseId: string } }) {
  ...
  const { data: liveSessionRows } =
    liveSessionModuleIds.length > 0
      ? await supabase.from("learning_live_sessions").select("module_id, scheduled_start, is_webinar").in("module_id", liveSessionModuleIds)
      : { data: [] };
  const liveSessions = Object.fromEntries(
    (liveSessionRows ?? []).map((r) => [r.module_id, { scheduledStart: r.scheduled_start, isWebinar: r.is_webinar }])
  );
  ...
  return (
    <>
      ...
      <div className="flex-1 space-y-6 px-8 py-6">
        <CourseEditor course={course} modules={modules ?? []} liveSessions={liveSessions} />
