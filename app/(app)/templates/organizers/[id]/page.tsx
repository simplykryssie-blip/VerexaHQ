...
export default async function OrganizerBuilderPage({ params }: { params: { id: string } }) {
  ...
  const [{ data: template }, { data: fields }] = await Promise.all([
    ...
    supabase
      ...
      .select(
        "id, organizer_template_id, parent_field_id, field_type, label, help_text, body_html, display_order, is_required, options, conditional_logic, client_profile_field, relationship_role, layout_width, include_in_document_checklist, document_checklist_name, document_checklist_category"
      )
