...
// Everything the full client detail page and the Quick-View drawer both
// need to render <ClientWorkspace> -- pulled out of page.tsx so the drawer
// (app/(app)/@modal/(.)clients/[id]/page.tsx) can fetch the exact same data
// instead of a second, drifting copy of ~20 queries. Returns null when the
// workspace or client can't be resolved; callers decide how to handle that
// (the full page 404s, the drawer just renders nothing).
export async function getClientWorkspaceData(clientId: string): Promise<ClientWorkspaceProps | null> {
  ...
  // rather than assuming there's only ever one. A .maybeSingle() query here
  // would throw once any client had two concurrent active runs, which is an
  // expected, supported state, not an edge case.
