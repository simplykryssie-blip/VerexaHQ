...
import { isWorkspaceStatusOperational, isSuspensionRecoveryPath, workspaceOperationalError } from "@/lib/workspace";

describe("isWorkspaceStatusOperational", () => {
  it("treats active as operational", () => {
    expect(isWorkspaceStatusOperational("active")).toBe(true);
  });

  it("treats suspended, archived, and permanently_archived as non-operational", () => {
    expect(isWorkspaceStatusOperational("suspended")).toBe(false);
    expect(isWorkspaceStatusOperational("archived")).toBe(false);
    expect(isWorkspaceStatusOperational("permanently_archived")).toBe(false);
  });
});
