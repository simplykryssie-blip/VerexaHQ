-- Phase 2: Firm Configuration Engine -- bugfix
-- duplicate_config_object() stripped created_at/updated_at from the jsonb
-- row before jsonb_populate_record(), which leaves those columns literally
-- NULL (jsonb_populate_record does not fall back to column defaults for
-- keys that are simply absent from the input) instead of a fresh
-- timestamp -- violating the NOT NULL constraint on every top-level table.
-- Fix: set them explicitly instead of stripping them.
