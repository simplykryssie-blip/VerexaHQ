-- Phase 2: Firm Configuration Engine -- Blueprint Application + Case Sharing
--
-- First, a correctness fix to duplicate_config_object() from 0028: as
-- originally written it preserved the SOURCE row's workspace_id, which for a
-- Verexa system object (workspace_id null) meant the "duplicate" was itself
-- inserted as ANOTHER global system row -- any authenticated user could have
-- created rows with workspace_id null in a shared, unscoped table. Copying a
-- system template must always land inside a specific workspace.
...
-- ============================================================================
-- apply_blueprint: the "Use Verexa Blueprint" / "Browse Blueprint Library"
-- onboarding path. Deep-copies every component of a blueprint into the
-- workspace, then re-points the copied service(s) at the copies of their
-- process/pipeline/organizer/document request/engagement letter/pricing/
-- billing rule -- never at the original system rows.
-- ============================================================================
...
-- ============================================================================
-- Case Sharing. A PTIN never exposes its full client list -- it shares one
-- Case at a time with a connected ERO, choosing exactly what to share.
-- case_id has no foreign key yet: Jobs/Cases don't exist until Phase 3. The
-- constraint will be added there; the sharing mechanics are independent of
-- it and don't need to wait.
-- ============================================================================
...
-- security-definer RPCs below, which enforce the connection + role checks.
