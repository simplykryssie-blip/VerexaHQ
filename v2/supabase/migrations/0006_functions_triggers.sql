...
-- Security-definer helper functions (used by RLS policies to avoid recursive
-- policy checks), updated_at triggers, the generic audit trigger, and the
-- auth.users -> user_profiles provisioning trigger.

-- ============================================================================
-- Helper functions. All are STABLE + SECURITY DEFINER, owned by the migration
-- role (table owner), so they read past RLS deliberately and safely: they are
-- the only sanctioned way policies check membership/permissions.
-- ============================================================================
...
-- ============================================================================
-- Generic audit trigger: writes an audit_log row for every INSERT/UPDATE/
-- DELETE on the tables it is attached to. Runs as table owner (bypasses RLS)
-- so it can log regardless of the acting user's row-level access.
-- ============================================================================
...
-- ============================================================================
-- auth.users -> user_profiles provisioning
-- ============================================================================
