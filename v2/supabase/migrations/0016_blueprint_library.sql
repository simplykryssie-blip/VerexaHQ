...
-- Blueprint Library. A blueprint bundles together a full configured offering
-- (service + process + pipeline + organizer + document requests +
-- engagement letter + pricing/billing rules + automation + dashboard).
--
-- workspace_id NULL = a Verexa system blueprint: read-only, never editable,
-- visible to every workspace as a starting point. workspace_id set = a
-- workspace's own copy (or a custom bundle it assembled itself), fully
-- editable and independent of the original.
--
-- blueprint_components references its bundled objects polymorphically
-- (component_type + component_id) rather than a hard foreign key, because a
-- single blueprints table cannot FK into a dozen different component
-- tables. Referential integrity for each component_type is enforced by the
-- apply_blueprint()/duplicate_config_object() RPCs, not the database schema.
