-- Revision Sprint -- Phase 2.5: Compliance Dashboard reporting views.
--
-- Plain (non security-definer) views over existing workspace-scoped tables:
-- Postgres evaluates the querying user's own RLS on the underlying tables
-- for every view select, so these need no additional access logic and
-- can't leak across workspaces -- a workspace admin sees only what their
-- existing audit_log/login_history/etc. RLS already allows them to see.
-- inactive_users is a function, not a view, because "how long is inactive"
...
-- ============================================================================
-- compliance_inactive_users: parameterized so the inactivity threshold is
-- never hardcoded. Runs as invoker (default), so RLS on workspace_users/
-- user_profiles still applies to the caller.
-- ============================================================================
