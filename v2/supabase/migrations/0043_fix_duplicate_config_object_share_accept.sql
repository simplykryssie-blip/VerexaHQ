-- Revision Sprint -- bugfix: duplicate_config_object() vs. share accept
--
-- 0034 added a same-workspace guard to duplicate_config_object() to stop an
-- arbitrary admin from copying another workspace's private object into
-- their own (the function runs SECURITY DEFINER, so its internal SELECT
-- bypasses RLS and can read any row regardless of caller). That guard was
-- too strict: it also blocked accept_config_object_share()'s entirely
-- legitimate cross-workspace copy. The fix narrows the guard instead of
-- removing it: a cross-workspace copy is allowed only when a pending
-- config_object_shares row specifically grants the destination workspace
-- access to that exact object -- i.e. only through share_config_object() +
-- accept_config_object_share(), never a bare cross-workspace call.
