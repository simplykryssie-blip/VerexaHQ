-- Mirrors the run.document_signed pattern added for send_document_for_
-- signature: create_task now records the task it creates into
-- trigger_snapshot.created_tasks (keyed by the creating step's id), and a
-- new run.task_completed condition (composite "<step_id>|<true|false>"
-- value, same shape as run.document_signed/client.organizer_status) reads
-- that task's live status. Lets a workflow branch on "did staff finish the
-- task I created two steps ago" -- e.g. a manual banking-approval review --
-- without needing a client/engagement-scoped field for it.
