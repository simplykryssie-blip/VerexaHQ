-- Gap #4 from the GHL capability audit: Assign Staff always required one
-- fixed person chosen at design time. Adds action_config.assignment_mode
-- ('fixed', default, unchanged behavior via staff_id) vs 'round_robin' --
-- resolves at run time to whichever eligible staff member (action_config.
-- staff_pool, or everyone active if empty) currently has the fewest open
-- engagements (assigned_staff_id, status not Completed/Archived) or open
-- clients (relationship_manager_id, lifecycle_status not archived/lost),
-- matching whichever target this step assigns. Ties broken randomly so
-- assignments spread out evenly over time.
