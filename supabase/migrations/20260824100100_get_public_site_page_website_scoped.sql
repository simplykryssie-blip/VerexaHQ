-- get_public_site_page now resolves through a website (a workspace can run
-- more than one), and returns the website's favicon/tracking scripts so the
-- public route can render them. Signature changes from
-- (p_workspace_slug, p_page_slug) to (p_workspace_slug, p_website_slug, p_page_slug).
...
revoke all on function public.get_public_site_page(text, text, text) from public;
grant execute on function public.get_public_site_page(text, text, text) to anon, authenticated;
