-- Activating an automation with a step that's missing required config (an
-- organizer, an email/SMS template, a target stage, etc.) has always just
-- silently done nothing useful the first time that step ran, with only a
-- generic Postgres error surfacing much later in automation_execution_logs.
-- This lets the "Activate" toggle check first and refuse with a specific,
-- fixable reason instead. Automations are a strictly linear step sequence
-- (display_order only, no branch/condition-per-step column), so there is no
-- "invalid workflow branch" concept to validate yet.
create or replace function public.validate_automation(p_automation_id uuid)
returns table(step_order int, action_type text, display_name text, issue text)
language plpgsql
security definer
...
begin
  ...
  for v_step in select * from public.automation_steps where automation_id = p_automation_id order by display_order loop
    if v_step.action_type = 'send_organizer_template' then
      if nullif(v_step.action_config->>'organizer_template_id', '') is null then
        return query select v_step.display_order, v_step.action_type, coalesce(v_step.display_name, 'Send Organizer'), 'No organizer is configured for this step.';
      elsif not exists (select 1 from public.organizer_templates where id = (v_step.action_config->>'organizer_template_id')::uuid) then
    ...
    elsif v_step.action_type = 'send_notification' and nullif(v_step.action_config->>'staff_id', '') is null then
      return query select v_step.display_order, v_step.action_type, coalesce(v_step.display_name, 'Notify Staff'), 'No recipient is selected for this step.';
