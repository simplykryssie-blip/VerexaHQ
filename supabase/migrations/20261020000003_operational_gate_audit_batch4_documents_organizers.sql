-- P1: workspace-operational-gate audit -- batch 4 (document requests,
-- organizer information requests, signature workflow, installed templates).
--
-- Judgment calls applied consistently with earlier batches:
-- * "Propose"/"save reopened answer" functions that only ever queue a
--   proposed_value pending staff approval (propose_organizer_answer_correction,
--   save_organizer_dynamic_required_answer, save_organizer_reopened_field_answer)
--   are NOT gated here, same as propose_client_sensitive_field -- the real
--   mutation happens on staff approval, which IS gated
--   (approve_organizer_information_request_item).
-- * reveal_organizer_answer is a read + audit-log-only action, same as
--   reveal_client_ssn/ein/itin -- not gated.
-- * reject/unflag/cancel-type actions (reject_organizer_information_request_item,
--   unflag_organizer_information_request_item, cancel_signature_request,
--   mark_organizer_information_request_viewed) close out or dismiss an
--   existing queue item without creating new real data -- not gated, same
--   as reject_client_pending_change.
-- * Routine config edits (add/remove) ARE gated regardless of direction,
--   matching the pipeline-stage-editing precedent: delete_installed_template
--   and reorder_organizer_fields are workspace configuration changes, not
--   relationship/seat/billing exposure reduction, so they're gated like
--   delete_process_stage was.
