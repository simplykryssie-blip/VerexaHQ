-- User directive: "Pipelines should be stage based only, no automations."
-- The per-stage organizer/document-request/engagement-letter pre-selects
-- were purely a "manual send convenience" (the UI itself said "doesn't
-- send anything automatically"), and every one of those three actions
-- already has a full, independent template-picker UI elsewhere on the
-- engagement page (QuickActions.tsx for organizers, RequestsPanel.tsx for
-- document requests, SignaturesPanel.tsx for engagement letters) -- so
-- this was a redundant duplicate mechanism, not a capability. Removing it
-- entirely: pipelines become pure name + order + tasks.
--
-- Two functions clone process_stages and explicitly copy these 3 columns
-- (duplicate_config_object for 'processes'/'services', and
-- turn_on_service) -- redefined first, minus those columns, so the column
-- drop below doesn't break them.
...
alter table public.process_stages
  drop column organizer_template_id,
  drop column document_request_template_id,
  drop column engagement_letter_template_id;
