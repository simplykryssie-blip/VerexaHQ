-- create_task and send_notification inserted action_config's title/
-- description/message verbatim -- unlike send_email/send_sms, which go
-- through notification_queue + the dispatch-notifications worker's
-- renderTemplate() -- so a step configured with "Review {{client_name}}"
-- literally produced that unrendered string in tasks.title and in the
-- notification bell. Both actions already have the same v_context this
-- function builds for send_email/send_sms; this just also applies it to
-- them, and to the plain-text values these two actions actually own.
  ...
  begin
    if v_step.action_type = 'delay' then
      ...
      values (
        ...
        case when v_run.engagement_id is not null then 'engagement' else 'client' end,
        coalesce(v_run.engagement_id, v_run.client_id),
        public.render_merge_fields(coalesce(v_step.action_config->>'body', ''), v_context),
        true
      );
