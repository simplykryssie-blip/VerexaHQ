-- Multi-tag support for the client.tag_added trigger and the
-- add_tag/remove_tag actions. Both now prefer a jsonb array at
-- trigger_config->'tags' / action_config->'tags', falling back to the
-- legacy single trigger_config->>'tag' / action_config->>'tag' string when
-- no array is present -- so every automation saved before this migration
-- keeps firing/executing exactly as before, with no data backfill needed.

-- 1. Trigger matching: fire if the newly-added tag matches either the
-- legacy single "tag" or is present in the "tags" array.
...
-- 2. add_tag/remove_tag execution: accept action_config->'tags' (jsonb
-- array) in addition to the legacy action_config->>'tag' (single string).
...
-- 3. rename_workspace_tag: also cascade into the "tags" array shape.
...
-- 4. delete_workspace_tag: also block deletion when referenced from a
-- "tags" array, not just the legacy singular "tag".
...
-- 5. list_workspace_tags_with_usage: count usage from the "tags" array too.
...
begin
  ...
  select
    ...
    coalesce((
      ...
      where a.workspace_id = p_workspace_id
        and (
          (a.trigger_type = 'client.tag_added' and (a.trigger_config->>'tag' = wt.name or a.trigger_config -> 'tags' ? wt.name))
          or exists (
            select 1 from public.automation_steps s
            where s.automation_id = a.id and s.action_type in ('add_tag', 'remove_tag')
              and (s.action_config->>'tag' = wt.name or s.action_config -> 'tags' ? wt.name)
          )
          or exists (
            select 1 from public.automation_step_edges e, jsonb_array_elements(coalesce(e.branch_conditions, '[]'::jsonb)) as cond
