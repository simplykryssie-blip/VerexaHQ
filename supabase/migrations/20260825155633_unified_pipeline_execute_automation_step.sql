-- Full redefinition of execute_automation_step (established convention for
-- this function). Four changes from the previous version:
-- 1. move_lead_stage + move_engagement_stage collapsed into one
--    move_pipeline_stage action, reading/writing the unified
--    pipeline_runs/pipeline_stages tables. No longer errors when an active
--    run exists in a different pipeline -- same permissive concurrent-
--    pipeline model already shipped for leads.
-- 2. change_stage's two branches (lead vs engagement) collapsed into one,
--    picking the most-recently-started active run for whichever entity is
--    on this automation run (now that more than one can exist at once).
-- 3. move_lead_to_service_pipeline now calls start_pipeline_run and reads
--    the unified table.
-- 4. create_engagement now hands off an existing active same-pipeline run
--    (client -> engagement) instead of always starting a brand-new one --
--    this is what lets one pipeline span the lead phase and the engagement
--    phase continuously.
  ...
  begin
    if v_step.action_type = 'delay' then
      ...
      values (
        ...
        coalesce(v_step.action_config->>'title', 'Appointment'),
        nullif(v_step.action_config->>'description', ''),
      ...
      if v_service.process_id is not null then
        -- Hand off the client's existing pipeline run (if one is active in
        -- the SAME pipeline the engagement resolves to) instead of starting
        -- a second, brand-new run from stage 1 -- this is what lets one
        -- pipeline span both the lead phase and the engagement phase
        -- continuously.
      ...
      -- Whichever entity this automation run is for, advance its most
      -- recently started active pipeline run's current stage -- this
      -- action has no pipeline picker in its config, so when more than one
      -- concurrent active run exists it can't disambiguate further.
      if v_run.engagement_id is not null then
      ...
      -- Only looks at (and only ever no-ops against) a run already in THIS
      -- step's own target pipeline -- an active run in a different
      -- pipeline belongs to a concurrent service/case and is left
      -- untouched.
      ...
      -- Prefer the specific service that triggered this run (populated on
      -- automations fired per-service-interest) so a client with more than
      -- one simultaneous service interest lands in the right pipeline
      -- instead of whichever interest happens to be most recently created.
      v_resolved_service_id := coalesce(
        nullif(v_run.trigger_snapshot->>'service_id', '')::uuid,
        (
          select service_id
      ...
      -- Only same-pipeline re-entry is a no-op; an active run in a
      -- different pipeline (a concurrent service) is left alone.
...
-- Fold existing automations built with the old action-type names into the
-- new unified one, so already-configured workflows (including "Phase 1
-- Step 1: New Leads Enter CRM") keep working without staff rebuilding
-- anything.
update public.automation_steps set action_type = 'move_pipeline_stage' where action_type in ('move_lead_stage', 'move_engagement_stage');
