-- Root cause of "DELETE requires a WHERE clause" on duplicate_config_object
-- and turn_on_service: the `authenticator` Postgres role (which PostgREST
-- connects as for every real API request) has a per-role
-- `session_preload_libraries = 'supautils, safeupdate'` override
-- (`pg_db_role_setting`), which is invisible when checking the global
-- session_preload_libraries GUC or when reproducing via `set local role`
-- in an existing superuser session -- role-level SET config only applies
-- on a fresh authenticated connection, which is exactly why every direct
-- SQL reproduction attempt succeeded while the real app path failed.
--
-- safeupdate blocks any DELETE/UPDATE with no WHERE clause, as a literal
-- syntax check. Both functions intentionally clear a per-transaction temp
-- table (`delete from tmp_x;`, no WHERE, by design) before repopulating
-- it. Adding `where true` satisfies the check without weakening safeupdate
-- for any other, genuinely-unfiltered statement elsewhere in the app.
