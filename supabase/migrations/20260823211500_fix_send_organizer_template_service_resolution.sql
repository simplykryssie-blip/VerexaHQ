-- send_organizer_template's auto-detect only ever looked at
-- trigger_snapshot->>'service_id', which is populated by
-- fire_service_interest_automations() but NEVER by fire_lead_created_automations()
-- (the lead.created trigger). Any automation that reaches this step after a
-- lead.created trigger -- e.g. "New Leads Enter CRM" -- always resolved a
-- null organizer_template_id and blew up on organizer_responses' NOT NULL
-- constraint with a raw, unhelpful DB error.
--
-- Fix: fall back to the client's most recent client_service_interests row
-- (same pattern already used by move_lead_to_service_pipeline) when
-- trigger_snapshot has no service_id, and raise a clear application-level
-- error if a template still can't be resolved instead of hitting the
