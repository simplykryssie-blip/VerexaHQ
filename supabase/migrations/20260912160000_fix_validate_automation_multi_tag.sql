-- validate_automation() still only checked the legacy single action_config->>
-- 'tag' field for add_tag/remove_tag steps, even though execute_automation_step
-- and the workflow builder's TagListInput UI were already updated (multi_tag_
-- trigger_and_actions migration) to store multiple tags in action_config->
-- 'tags' (a jsonb array). A step configured with one or more tags via the
-- current UI has no 'tag' key at all, so validate_automation always flagged
-- it "No tag is configured for this step" -- blocking activation/publish
-- every time, even though the tags were saved correctly. This is what
-- actually broke "add multiple tags" and made newly added tags look like
-- they weren't saving: the step itself saved fine, only turning the
-- workflow on kept rejecting it.
