-- Phase 8 follow-up: send_document_request (the automation action) and
-- document_request.completed (the automation trigger) were both hardcoded
-- to engagements only, unlike document_request.sent and document.uploaded
-- (which already correctly handle entity_type 'client' or 'engagement'),
-- and unlike the manual "New document request" panel, which has supported
-- sending to a lead/client directly since Phase 7. Neither the action's nor
-- the trigger's label says "for an engagement", so this was an unexplained
-- inconsistency rather than an intentional scope limit.
--
-- send_document_request now targets whichever of engagement_id/client_id
-- the run has (same "engagement if present, else client" pattern used
-- throughout execute_automation_step), including in the partial-
-- fulfillment carryover lookup, which previously only ever matched prior
-- engagement-scoped requests.
--
-- fire_document_request_completed_automations now also fires for
-- entity_type='client': the service_id needed for the trigger's per-service
-- filter is resolved from the client's most recent client_service_interests
-- row when there's no engagement to read it from directly -- the same
-- resolution already used by send_organizer_template and
-- move_lead_to_service_pipeline elsewhere in this engine.
