-- Threads is_test through execute_automation_step: every client-visible
-- action type (send_email, send_sms, send_portal_message,
-- send_engagement_letter, invite_to_portal, webhook, send_quote) is
-- skipped and logged with a skipped_reason instead of actually queuing a
-- real send/document/portal account when the run is a test run. Every
-- other action type is untouched -- it still executes for real, against
-- whichever client/engagement run_automation_test's caller picked, which
-- is what makes the test worth trusting rather than a dry-run guess.
-- start_workflow additionally propagates is_test onto the child run it
-- starts, so a test run can't chain into a real, live-sending child
-- automation.
  ...
  begin
    if v_step.action_type = 'delay' then
      ...
      if v_pipeline_stage_id is distinct from v_target_stage_id then
        -- Jumping straight to the target stage still has to pass through
        -- every stage in between (pipeline_runs.current_stage_id only ever
        -- advances one stage at a time), but those in-between stages were
        -- never actually visited -- mark them Skipped rather than walking
        -- them through Completed one at a time, which used to briefly
        -- promote each to "In Progress" and fire its own stage-entered
        -- automations (e.g. approving an organizer used to also fire the
        -- "Missing Docs" and "ERO Review Declined" stage automations on the
        -- way to "Ready for Preparation"). Skipped in descending order so
        -- advance_pipeline_on_stage_completed's "next pending stage" search
        -- never lands on one of them; marking the current stage Completed
        -- last lets that same search resolve straight to the real target.
      ...
      -- A client landing in a non-funnel pipeline has clearly moved past
      -- any lead-funnel pipeline they were still sitting in -- close it out
      -- directly rather than promoting through its own remaining stages
      -- (which would refire every intermediate stage's automations).
      -- Processed in descending display_order so the trigger never finds a
      -- "next pending stage" to promote to In Progress.
      ...
      -- Same funnel-closure as move_pipeline_stage above -- this action's
      -- entire purpose is graduating a lead out of intake into their real
      -- service pipeline, so it's exactly where a dangling funnel run needs
      -- to be closed.
