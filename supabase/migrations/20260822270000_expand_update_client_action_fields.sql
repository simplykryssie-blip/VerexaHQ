-- GHL's "Update Contact Field" workflow action covers essentially any
-- contact field; ours only covered 4 (first_name, last_name,
-- primary_phone, relationship_manager_id). Expands update_client to the
-- rest of the commonly-useful, non-sensitive client fields: name parts,
-- business name, client type, email (kept in sync with normalized_email
-- the same way primary_phone already syncs normalized_phone), and the
-- mailing address fields.
--
-- Deliberately excluded: date_of_birth/SSN/EIN/ITIN (guard_client_sensitive_fields()
-- requires an acting user's clients.edit_sensitive permission, which an
-- automation run -- no acting auth.uid() -- can never satisfy, so a step
-- targeting one of these would always fail), lifecycle_status (already
-- has dedicated mark_lead_lost/convert_lead_to_client actions with their
-- own side effects), and tags (already has dedicated add_tag/remove_tag
-- actions).
