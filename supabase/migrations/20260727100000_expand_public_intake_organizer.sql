-- Expand the public intake organizer's conditional logic to match the
-- full Individual Tax Preparation questionnaire (spouse, dependents,
-- per-income-type detail, deductions/credits, life changes). None of this
-- needs new columns or a new answers-merge mechanism -- save_intake_progress
-- already merges the *entire* current answers object into the jsonb
-- `answers` column on every save (see save_intake_progress), so every new
-- nested key below (answers.spouse, answers.dependents,
-- answers.self_employment_detail, etc.) already round-trips correctly.
-- This migration only has to teach the two derived-state functions to
-- read those new keys. Both functions still recompute everything from
-- scratch on every call, so an answer a client later toggles off simply
-- stops contributing on the next save -- it never lingers as "active" in
-- either the checklist or the complexity/flag scoring.
...
begin
  ...
  -- Dependents (form 8332 / IP PIN on any dependent)
  ...
  -- Other income / deductions worth flagging for staff attention
  ...
  -- Life changes -> tax compliance (audit/notice/bankruptcy/collections)
...
begin
  ...
  if coalesce((a #>> '{deductions,property_taxes}')::boolean, false) or coalesce((a #>> '{deductions,real_estate_taxes}')::boolean, false) then
  ...
  if coalesce((a #>> '{life_changes,family,divorce}')::boolean, false) then
...
-- ---------------------------------------------------------------------
-- public_update_document_request — anon-callable, token-validated. Lets a
-- taxpayer mark their own checklist item as "not applicable", "waiting on
-- it", or attach a short explanation -- but only ever within their own
-- intake (the token resolves the intake, and the target row is re-checked
-- against that same intake_id) and only ever into a taxpayer-safe status.
-- Every staff-only transition (accepted, under_review, replacement_needed,
-- waived_by_staff) is intentionally excluded from p_status's allowed set.
-- ---------------------------------------------------------------------
create or replace function public.public_update_document_request(
  p_token text,
  p_document_request_id uuid,
  p_status text,
  p_client_note text default null
)
returns public.intake_document_requests
language plpgsql
security definer
...
begin
  if p_status not in ('requested', 'waiting_on_client', 'not_applicable') then
    raise exception 'That status is not available here.';
  end if;
  ...
  if not exists (select 1 from public.intake_document_requests where id = p_document_request_id and intake_id = v_row.intake_id) then
    raise exception 'That document request does not belong to this intake.';
  end if;

  update public.intake_document_requests
  set status = p_status, client_note = p_client_note, updated_at = now()
  where id = p_document_request_id and intake_id = v_row.intake_id;

  return (select d from public.intake_document_requests d where d.id = p_document_request_id);
end;
$function$;

revoke execute on function public.public_update_document_request(text, uuid, text, text) from public;
grant execute on function public.public_update_document_request(text, uuid, text, text) to anon, authenticated;

-- ---------------------------------------------------------------------
-- public_intake_documents — anon-callable, token-validated read of a
-- single intake's own uploaded-file list (filename/type/timestamp only --
-- never storage_path, so the browser never learns the private bucket
-- layout even though it's already a non-public bucket).
-- ---------------------------------------------------------------------
create or replace function public.public_intake_documents(p_token text)
returns table (id uuid, document_request_id uuid, original_file_name text, mime_type text, uploaded_at timestamptz)
language plpgsql
security definer
set search_path to 'public, extensions'
as $function$
...
begin
  ...
  return query
    select d.id, d.document_request_id, d.original_file_name, d.mime_type, d.uploaded_at
    from public.intake_documents d
    where d.intake_id = v_row.intake_id
    order by d.uploaded_at;
end;
$function$;

revoke execute on function public.public_intake_documents(text) from public;
grant execute on function public.public_intake_documents(text) to anon, authenticated;
