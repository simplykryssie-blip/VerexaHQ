-- Phase 8 (automations): execute_automation_step's send_email and send_sms
-- branches hardcoded entity_type = 'engagement' and entity_id =
-- v_run.engagement_id on the notification_queue row they insert, even when
-- the automation run is client-only (a lead, before any engagement exists --
-- exactly the "New Leads Enters CRM" trigger's normal case). That leaves
-- entity_id null and entity_type wrong for every such send.
--
-- Confirmed live: creating a real lead and letting "New Leads Enters CRM"
-- run through send_email/send_sms produced notification_queue rows with
-- entity_type='engagement', entity_id=null. app/api/cron/dispatch-
-- notifications/route.ts's resolveClientIdFromContext() can't resolve a
-- client from a null entity_id, so resolvePortalMergeFieldsFromContext()
-- always returns portalInviteLink=null for these -- any email/SMS template
-- used in a client-only automation that references {{portal_invite_link}}
-- will always fail to send with "no active, unexpired invitation exists
-- for this client", even when a real one does.
--
-- Fixed to use the same "engagement if present, else client" pattern
-- add_note and send_notification already use correctly elsewhere in this
-- same function.
