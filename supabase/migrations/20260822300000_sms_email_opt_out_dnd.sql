-- clients had zero consent/opt-out tracking at all, and send_email/send_sms
-- only checked whether primary_email/primary_phone was non-null before
-- sending -- real TCPA/CAN-SPAM exposure for a firm auto-texting/emailing
-- clients with no way to honor an opt-out. Adds sms_opt_out/email_opt_out
-- (plus _at timestamps for an audit trail of when), gates send_email/
-- send_sms in execute_automation_step on them, and adds add_dnd/remove_dnd
-- actions so a workflow can also opt someone in/out explicitly.
--
-- An opted-out send is a skip, not a failure -- the step still completes
-- and the run continues, same as invite_to_portal's already-invited no-op,
-- rather than halting the whole workflow just because one send was
-- suppressed. The skip is recorded in automation_execution_logs'
-- execution_data for visibility.

alter table public.clients add column sms_opt_out boolean not null default false;
alter table public.clients add column sms_opt_out_at timestamptz;
alter table public.clients add column email_opt_out boolean not null default false;
alter table public.clients add column email_opt_out_at timestamptz;
...
alter table public.automation_steps add constraint automation_steps_action_type_check
  check (action_type = ANY (ARRAY[
    ...
    'end_workflow', 'invite_to_portal', 'condition', 'create_appointment', 'add_dnd', 'remove_dnd'
  ]));
