-- Lets a builder mark a question as staff-only -- e.g. an internal routing
-- note or reminder embedded in the organizer's structure that should never
-- be shown to (or answerable by) the client filling it out. Additive and
-- defaults to false so every existing field keeps rendering exactly as it
-- does today.
alter table public.organizer_fields add column is_internal_only boolean not null default false;

-- Public link fill (app/o/[token]) must never even receive an internal-only
-- field's definition, not just hide it client-side -- redefine to exclude it
-- from the fields array this RPC returns.
...
begin
  ...
  select jsonb_build_object(
    ...
    'fields', coalesce((
      ...
      where f.organizer_template_id = v_template.id and f.is_internal_only = false
    ), '[]'::jsonb)
  ) into v_result;
