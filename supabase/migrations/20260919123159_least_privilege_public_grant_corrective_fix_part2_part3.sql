...
-- 2026-09-19 (recorded version 20260919123159, name
-- least_privilege_public_grant_corrective_fix_part2_part3 in
...
-- (27242 bytes, confirmed exact match). Confidence: A -- exact original
...
-- fresh project.
--
-- LINEAGE: this is the corrective fix for the second group of an earlier,
-- superseded pass (least_privilege_security_definer_cleanup_part2 at
-- 20260919081528, covering the same 86 internal/trigger functions below).
-- That migration is NOT separately recovered here -- it is Class C
-- (superseded): it revoked anon and authenticated execute but never
-- revoked PUBLIC, leaving the same class of leak this migration exists to
-- close. This migration revokes from PUBLIC, anon, and authenticated for
-- all 86 functions below (locking them to service_role only, matching
-- their role as internal/trigger-only functions never meant to be called
-- directly), plus revokes all anon/authenticated privileges on 4 tables
-- (rate_limit_hits, workspace_ghl_connections, workspace_jotform_
-- connections, workspace_partner_purchase_webhooks). It fully supersedes
-- and subsumes the earlier pass on its own, with no dependency on it
-- having been applied first.
--
-- CURRENT-STATE VERIFICATION (2026-09-20, read-only): re-checked
-- has_function_privilege for anon/authenticated/public/service_role
-- against all 86 functions, and information_schema.role_table_grants for
-- the 4 tables, directly on production. All 86 functions and all 4 tables
-- match this migration's intended state exactly -- zero drift.
-- ============================================================================
