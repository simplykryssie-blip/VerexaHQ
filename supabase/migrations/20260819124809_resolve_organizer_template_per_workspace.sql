-- Resolve the organizer template to send by slug, scoped to the run's
-- workspace with a fallback to a global (workspace_id is null) template of
-- the same slug when the workspace doesn't have its own copy.
-- Superseded by 20260819131326_strict_workspace_only_organizer_template_resolution.sql,
-- which removes the global fallback -- kept here to preserve migration history.
        ...
        coalesce(
          ...
          (
            ...
            join public.organizer_templates ot
              ...
              and (ot.workspace_id = v_run.workspace_id or ot.workspace_id is null)
            where s.id = nullif(v_run.trigger_snapshot->>'service_id', '')::uuid
            order by (ot.workspace_id = v_run.workspace_id) desc
            limit 1
          )
        )
      );
