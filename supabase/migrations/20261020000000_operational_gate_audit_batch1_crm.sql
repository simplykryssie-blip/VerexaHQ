...
--
-- Same bug class as the already-fixed create_client/create_engagement: these
-- SECURITY DEFINER RPCs check has_permission(...) but never
-- is_workspace_operational(...), so they bypass their tables' own
-- (correctly-gated, where it exists) RLS policies and let a member with
-- permission keep mutating real client data in a suspended/archived/
-- permanently_archived workspace. Fix is the minimal, established pattern:
-- add the operational check right after the existing permission check, no
-- other logic changed.
--
-- For the four portal-client "propose_client_*" functions, only the
-- auto-APPLIED branch is gated (a client typing a change that gets queued
-- for staff review is not itself an operational mutation of real business
-- data -- the actual write only happens on auto-apply or on staff approval,
-- and staff approval is separately gated below via approve_client_pending_change).
