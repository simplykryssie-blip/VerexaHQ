-- "Notify a staff member" required hand-picking one specific staff_id and
-- always sent In-App only. Rewired to broadcast to the workspace owner plus
-- every active staff member automatically, with a choice of channels
-- (Staff portal / In-App, Email, or both) instead of always In-App.

insert into public.email_templates (workspace_id, name, slug, category, subject, body_html, merge_fields, status)
values (
  null, 'Automation Staff Notification', 'automation-staff-notification', 'internal',
  '{{firm_name}}: {{message}}',
...
begin
  ...
  begin
    if v_step.action_type = 'send_email' then
      ...
      -- Broadcasts to every active workspace member (owner included -- an
      -- owner is always also a workspace_users row) instead of one
      -- hand-picked staff_id, over whichever channels are configured
      -- (defaulting to In-App for automations built before this changed,
      -- so nothing that already relies on this action silently stops
      -- notifying anyone).
      v_channels := coalesce(
