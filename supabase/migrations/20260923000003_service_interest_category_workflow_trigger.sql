-- Extend the existing service-interest trigger so workflows can match either
-- a specific service or any service in a specific category. Legacy service_id
...
begin
  ...
  v_context := jsonb_build_object(
    ...
    'service_category_id', NEW.service_category_id,
    'source', NEW.source
  );
  ...
  for v_automation in
    ...
    where workspace_id = NEW.workspace_id
      ...
      and (
        trigger_config ->> 'service_id' = NEW.service_id::text
  ...
  loop
    if public.evaluate_automation_conditions(v_automation.conditions, v_context) then
      ...
      values (
        NEW.workspace_id, v_automation.id, v_engagement_id, NEW.client_id,
        v_context, 'running'
      )
...
-- Preserve the existing trigger-only security posture.
revoke all on function public.fire_service_interest_automations() from public, anon, authenticated;
