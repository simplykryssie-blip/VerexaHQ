-- JotForm/Cognito-style layout: lets a builder put fields side by side
-- instead of always one-per-row. 'full' (the default, and the only value
-- every existing field implicitly has today) keeps current behavior
-- unchanged; 'half'/'third'/'two_thirds' let a field share its row with
-- siblings. Structural field types (section/page_break/rich_text/
-- repeating_section) always render full-width regardless of this value --
-- enforced in the fill-surface components, not here, since a DB check
-- constraint can't reference field_type conditionally per row easily and
-- doing it in code keeps this migration purely additive.
alter table public.organizer_fields add column layout_width text not null default 'full';
alter table public.organizer_fields add constraint organizer_fields_layout_width_check check (
  layout_width = any (array['full', 'half', 'third', 'two_thirds'])
);
