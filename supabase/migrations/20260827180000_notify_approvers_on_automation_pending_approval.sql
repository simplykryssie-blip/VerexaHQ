-- requires_approval/approver_role_id on automation_steps and the
-- approve/reject_automation_step RPCs have existed since the automation
-- engine was built, but nothing ever notified anyone when a step actually
-- paused for approval -- an approver had no way to know to go look. This
-- makes start_next_automation_step notify the right people the moment it
-- inserts a pending_approval row: whoever holds the step's approver role,
-- or every workspace admin/owner if no specific role was configured.
