-- Conditions in a list were always implicitly AND'd together -- adding a
-- second condition to a branch (e.g. "portal status is not_sent" and
-- "portal status is invited") made the branch impossible to ever match,
-- since both can't be true at once. Each condition now carries an optional
-- `join` ("and" | "or") describing how it combines with the one before it,
-- folded strictly left to right (no AND-before-OR precedence -- simple
-- enough not to need parenthesized groups for a short visual list).
-- Missing `join` defaults to "and", so every condition list saved before
-- this change keeps behaving exactly as it did.
...
begin
  ...
  loop
    ...
    -- Folded strictly left to right (no AND-before-OR precedence) -- the
    -- first condition seeds the result, every condition after it combines
    -- with whatever came before using its own join operator. Can't
    -- short-circuit on a false AND anymore: a later OR could still flip
    -- the overall result to true.
    if v_index = 1 then
      v_result := v_match;
