-- send_notification ("Notify a staff member") used to broadcast to every
-- active staff member in the workspace regardless of configuration -- there
-- was no way to target one person. Adds a staff_id config field (surfaced in
-- the Workflows builder as a picker defaulting to the account owner) and
-- narrows execute_automation_step()'s send_notification branch to notify
-- just that one resolved staff member instead of looping over everyone.
...
begin
  ...
  begin
    if v_step.action_type = 'delay' then
      ...
      -- Targets exactly one staff member now, rather than broadcasting to
      -- every active staff account: the step's own staff_id when set,
      -- otherwise the workspace owner. If neither resolves to an active
      -- staff member (owner left/deactivated, no staff_id configured yet),
      -- this step is a silent no-op rather than an error -- same
      -- "skip, don't fail the run" treatment as an opted-out email/sms.
