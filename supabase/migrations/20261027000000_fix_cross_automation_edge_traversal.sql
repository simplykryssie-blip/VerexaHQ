-- P1 security fix: start_next_automation_step() followed automation_step_edges
-- purely by from_step_id, with no check that the edge's to_step_id actually
-- belongs to the run's own automation_id. automation_step_edges.to_step_id is
-- only a foreign key into automation_steps.id -- it does not require the
-- referenced step to share automation_id with from_step_id or with the edge's
-- own automation_id column. Nothing in the schema (no CHECK constraint, no
-- trigger), in RLS (the automation_step_edges write policies only check
-- has_permission(automations.workspace_id, 'automations.manage') on the
-- edge's own automation_id -- a standard, non-privileged permission any
-- workspace admin/owner already holds -- and validate nothing about
-- from_step_id/to_step_id), or in application code enforced that invariant.
--
-- Live reproduction (disposable fixtures, since cleaned up) proved this is
-- constructible through a fully ordinary, RLS-authenticated path: a
-- workspace admin with only their own automations.manage permission can
-- insert an automation_step_edges row whose to_step_id points at a step
-- belonging to a different automation -- including one owned by a different
-- workspace entirely -- and start_next_automation_step() will actually
-- follow it, silently executing the foreign step's action (proved with a
-- webhook action, is_test=true) while automation_runs.workspace_id/
-- automation_id and automation_execution_logs continue to attribute
-- everything to the original run's automation/workspace throughout.
--
-- Fix: scope the edge-selection loop and its paired dead-end/retry
-- existence check to only consider edges whose destination step belongs to
-- the run's own automation_id. This is the single point where edges are
-- resolved for traversal; execute_automation_step() is only ever invoked
-- with a step_id that originated from this same resolution (directly, or
-- via an automation_pending_steps row this function inserted), so fixing
-- this one function closes the gap with no need for a duplicate check
-- elsewhere. automation_id -> workspace_id is a fixed, non-reassignable
-- relationship, so matching automation_id transitively guarantees workspace
-- equality without a separate, redundant workspace_id check.
--
-- Verified against all real (non-test) data in this project: zero existing
-- automation_step_edges rows have a to_step whose automation_id differs from
-- its from_step's automation_id, and zero existing automation_runs rows have
-- a current_step_id outside their own automation_id -- so this change is a
-- structural no-op for every legitimate edge and run in the system today,
-- and only excludes malformed/cross-automation edges. No other behavior
-- (branch/If-Else evaluation, Wait/Delay scheduling, retry-until-matched,
-- approval gating, client or partner automation semantics) is touched.
