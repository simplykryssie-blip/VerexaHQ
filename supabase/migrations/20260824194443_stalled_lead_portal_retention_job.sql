-- Track when a client actually went lost, so the 30-day portal retention
-- window has something real to count from (updated_at is too noisy --
-- any unrelated edit after disengagement would keep pushing it out).
alter table public.clients add column if not exists lost_at timestamptz;

-- mark_lead_lost now stamps lost_at alongside lifecycle_status/lost_reason.
...
-- The actual retention job: revoke portal access for anyone whose lead
-- went lost 30+ days ago and still has active portal access. Idempotent
-- (only touches status = 'active' rows), matching the cron-RPC pattern
-- used by fire_task_overdue_automations et al.
...
begin
  with expired as (
    select cpu.id, cpu.workspace_id, cpu.client_id
    from public.client_portal_users cpu
    join public.clients c on c.id = cpu.client_id
    where cpu.status = 'active'
      and c.lifecycle_status = 'lost'
      and c.lost_at is not null
      and c.lost_at <= now() - interval '30 days'
  ),
  revoked as (
    ...
    returning cpu.id, cpu.workspace_id, cpu.client_id
  )
  ...
  select workspace_id, 'client_portal_users', id, 'update', 'info',
    ...
    jsonb_build_object('reason', '30-day stalled-lead portal retention period elapsed', 'client_id', client_id)
  from revoked;
