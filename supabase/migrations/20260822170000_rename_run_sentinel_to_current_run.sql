-- Naming cleanup, no behavior change for anything actually in use (nothing
-- in production stores the bare "run" sentinel yet, confirmed before this
-- migration was written): the Organizer status condition's "whichever
-- organizer this run sent" option used the bare string "run" as its
-- template-id sentinel, which reads as a typo or a stray value if anyone
-- ever inspects a condition's raw JSON. Renamed to "current_run" to be
...
begin
  ...
  loop
    ...
    if v_field = 'client.tags' then
      ...
      -- instead be the sentinel "current_run", meaning "whichever organizer
      -- this automation run itself sent" (recorded by the
      -- send_organizer_template step onto automation_runs.trigger_snapshot
      -- as last_organizer_template_id, which is how p_context is fed here).
      -- That's the only way to check this when the organizer is
      -- auto-detected per service rather than fixed.
