-- 1. Let the send_organizer_template action fall back to the triggering
--    service's own organizer_template_id when the step doesn't hardcode one.
...
begin
  ...
  begin
    if v_step.action_type = 'send_email' then
      ...
      -- If the step doesn't hardcode an organizer, fall back to whichever
      -- organizer the triggering service itself is linked to (services.organizer_template_id).
      -- This is what lets ONE automation route every service to its correct
      -- organizer instead of needing one automation per service.
      ...
      values (
        ...
        coalesce(
          ...
          (select s.organizer_template_id from public.services s where s.id = nullif(v_run.trigger_snapshot->>'service_id', '')::uuid)
        )
      );
...
-- 2. Let the trigger match a wildcard automation (no service_id in trigger_config)
--    against ANY service selection, not just an exact one.
