-- A client can legitimately have more than one active lead pipeline run at
-- once (e.g. Tax Resolution + Bookkeeping, or two simultaneous tax
-- engagements), mirroring client_service_interests which already allows
-- multiple simultaneous rows per client. Previously this table enforced at
-- most one Active run per client at all, so starting a second pipeline
-- silently completed/replaced whatever run was already active in a
-- different pipeline. Scope the uniqueness to (client_id, process_id)
-- instead -- a client still can't have two active runs in the SAME
-- pipeline, but concurrent runs in different pipelines are now allowed.
drop index if exists public.lead_pipeline_runs_one_active_idx;
create unique index lead_pipeline_runs_one_active_idx on public.lead_pipeline_runs (client_id, process_id) where (status = 'Active'::text);

-- Update the two action types that used to assume "at most one active lead
-- run per client" and would complete/replace an active run in a different
-- pipeline. Both now only look at (and only ever complete/no-op against) a
-- run already in the step's own target pipeline; an active run belonging to
-- a concurrent service's pipeline is left untouched.
...
begin
  ...
  begin
    if v_step.action_type = 'delay' then
      ...
      -- Only look at (and only ever no-op against) a run already in THIS
      -- step's own target pipeline -- an active run in a different
      -- pipeline belongs to a concurrent service and is left untouched.
      select id, current_stage_id into v_lead_run_id, v_lead_stage_id
      ...
      select id into v_lead_run_id
      from public.lead_pipeline_runs
      where client_id = v_run.client_id and status = 'Active' and process_id = v_target_process_id
      order by started_at desc limit 1;
