-- New standalone "business_hours_delay" step, kept deliberately separate
-- from the plain "delay" step per the owner: each step should have one
-- capability, not another wait_mode bolted onto "delay". action_config:
-- { "hours": <numeric> }. Scheduling (start_next_automation_step) computes
-- the deadline via compute_business_hours_deadline; execution
-- (execute_automation_step) is a no-op once the wait is over, matching how
-- "delay" and "condition" already work.
...
alter table public.automation_steps add constraint automation_steps_action_type_check
  check (action_type = any (array[
    ...
    'add_dnd','remove_dnd','move_lead_to_service_pipeline','business_hours_delay'
  ]));
...
-- execute_automation_step: 'business_hours_delay' is a no-op like 'delay'
-- and 'condition' -- the waiting itself already happened via scheduled_for
-- above, this just lets the run advance past it once the cron picks it up.
