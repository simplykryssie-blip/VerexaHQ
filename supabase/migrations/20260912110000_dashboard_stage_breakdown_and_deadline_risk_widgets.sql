-- Two new dashboard widget types: 'stage_breakdown' (donut of active
-- engagements by pipeline stage) and 'deadline_risk' (engagements
-- approaching their due date without enough stage progress). Both are
-- real, synchronously computed from existing engagements data -- no new
-- tables, same pattern as the top_services/engagement_pipeline widgets.
...
cross join (values ('stage_breakdown', 'Stage Breakdown', 1), ('deadline_risk', 'Deadline Risk', 2)) as w(widget_type, title, ord)
