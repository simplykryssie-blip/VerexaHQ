-- Condition-type automation steps can now opt into "keep checking until
-- matched" instead of permanently dead-ending the run the first time none
-- of their outgoing branches match. This mirrors the existing delay-step
-- wait_mode='until_condition' machinery (automation_pending_steps +
-- should_advance_wait_until_step + the every-minute
-- run-pending-automation-steps cron) rather than inventing new plumbing.
--
-- Opt-in per step via action_config:
--   { "retry_until_matched": true, "retry_timeout_days": 90 }
-- retry_timeout_days defaults to 90 and is a safety valve -- once exceeded,
-- the step falls through to the original dead-end behavior (logs a
-- dead_end execution log entry, completes the run) rather than retrying
-- forever.
--
-- Deliberately NOT a blanket engine-wide behavior change: a condition step
-- with no retry_until_matched set behaves exactly as before.
  ...
  if v_step.action_type = 'condition' then
    ...
    return exists (
      ...
      where e.from_step_id = v_step.id
        and (e.branch_conditions is null or public.evaluate_automation_conditions(e.branch_conditions, v_run.trigger_snapshot, v_run.workspace_id, v_run.client_id, v_run.engagement_id))
    );
  end if;
