-- duplicate_config_object() (the generic "clone this template into a
-- workspace" RPC, already whitelisting 'processes' and 'automations')
-- had three real gaps that made it unsafe to expose as a "Duplicate
-- pipeline" / "Duplicate workflow" button:
--
-- 1. It never copied automation_step_edges, and never captured an
--    old-step-id -> new-step-id map for automation_steps either, so
--    there was no way to add edge-copying without first fixing the
--    step copy. A duplicated workflow with a condition step silently
--    lost every branch (a straight-line auto-rebuild trigger papers
--    over this for a workflow with NO condition steps, but not for a
--    branching one).
-- 2. It copied automations.webhook_token verbatim from the source row.
--    webhook_token is UNIQUE, so duplicating any webhook-triggered
--    automation would fail outright on insert.
-- 3. It checked is_workspace_admin for every table, even though
--    pipelines and automations both now use a normal granted
--    permission (pipelines.manage / automations.manage) for every
--    other write path -- someone who can build a workflow via
--    automations.manage couldn't duplicate one.
--
-- automation_steps also gained canvas_x/canvas_y in the copy -- without
-- them every cloned step lands at the same default position, stacked
