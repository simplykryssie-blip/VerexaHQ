-- New automation action: move_lead_to_service_pipeline. Auto-resolves
-- the pipeline from whatever service the client most recently expressed
-- interest in (client_service_interests), the same auto-detect pattern
-- send_organizer_template already uses -- no pipeline is hardcoded into
-- the step, so it works for any of the 9 service pipelines without a
-- branch per service.
--
-- Needed for a real gap this surfaced: move_lead_stage refused to move a
-- lead onto a different pipeline than the one it's already active on
-- ("This lead is already in a different pipeline"), which made it
-- impossible for a lead to graduate off a generic intake pipeline (e.g.
-- "New Lead Pipeline") onto a service-specific one once their organizer
-- comes back -- exactly the flow being built. Fixed both here: switching
-- pipelines now closes out the old lead_pipeline_runs row instead of
-- blocking.
...
alter table public.automation_steps add constraint automation_steps_action_type_check
  check (action_type = any (array[
    'send_email','send_sms','send_notification','create_task','assign_user','change_stage','request_approval',
    'delay','webhook','escalate','send_organizer_template','create_engagement','send_engagement_letter',
    'send_document_request','move_lead_stage','mark_lead_lost','convert_lead_to_client','update_client',
    'create_client','move_engagement_stage','create_quote','send_quote','add_tag','remove_tag','add_note',
    'send_portal_message','start_workflow','end_workflow','invite_to_portal','condition','create_appointment',
    'add_dnd','remove_dnd','move_lead_to_service_pipeline'
  ]));
...
begin
  ...
  begin
    if v_step.action_type = 'delay' then
      ...
      if v_lead_run_id is not null and v_lead_run_process_id is distinct from (v_step.action_config->>'process_id')::uuid then
        -- Moving to a different pipeline than the lead's current one --
        -- close the old run instead of blocking, so a lead can graduate
        -- off a generic intake pipeline onto a different one.
        update public.lead_pipeline_runs set status = 'Completed', completed_at = now() where id = v_lead_run_id;
        v_lead_run_id := null;
      end if;
      ...
      select service_id into v_resolved_service_id
      ...
      select id, process_id into v_lead_run_id, v_lead_run_process_id
      from public.lead_pipeline_runs
      where client_id = v_run.client_id and status = 'Active'
      order by started_at desc limit 1;

      if v_lead_run_id is not null and v_lead_run_process_id is distinct from v_target_process_id then
        update public.lead_pipeline_runs set status = 'Completed', completed_at = now() where id = v_lead_run_id;
        v_lead_run_id := null;
      end if;

      if v_lead_run_id is null then
        perform public.start_lead_pipeline_run(v_run.client_id, v_target_process_id);
      end if;
