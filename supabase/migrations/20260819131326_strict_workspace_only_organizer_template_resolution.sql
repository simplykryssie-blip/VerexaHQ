-- Per explicit user instruction, pipelines, workflows/automations, engagement
-- letters, organizers, and email/SMS templates created in one workspace are
-- never automatically shared with or borrowed by another workspace. Remove
-- the "or ot.workspace_id is null" global fallback introduced in
-- 20260819124809_resolve_organizer_template_per_workspace.sql: if a
-- workspace has no organizer template of its own matching the triggering
-- service's slug, send_organizer_template now resolves to no template
-- (a no-op) rather than borrowing another workspace's content.
