-- automation_execution_logs.workflow_run_id has existed since this table
-- was created but execute_automation_step never populated it -- every row
-- has workflow_run_id = null, even though the same insert already puts
-- run_id inside execution_data as a jsonb field. That makes "show me every
-- log line for this specific run" impossible via an indexed join; you have
-- to text-match inside jsonb instead. Found live while QA-testing the "New
-- Leads Enters CRM" automation for the Admin AI QA Agent (Phase 2).
