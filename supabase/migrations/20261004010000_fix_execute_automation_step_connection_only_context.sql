-- execute_automation_step always references v_eng.* to build v_context, but
-- v_eng (an untyped `record`) is only ever assigned when the run has an
-- engagement_id or a client_id. A firm-connection-only run (no client, no
-- engagement -- exactly what firm_package.purchased and the new Phase 6B
-- partner_onboarding.created/.status_changed events produce) left v_eng
-- unassigned, and Postgres raises "record is not assigned yet" the moment
-- any field on it is read. This was already latent for firm_package.purchased
-- automations reaching a real step; Phase 6B's onboarding events are what
-- actually exercises it for the first time. Fix: give v_eng an all-null but
-- assigned shape in that case, identical column list to the other two
-- branches, so v_context still builds (with nulls for the fields that don't
-- apply to a connection-only run) instead of raising.
