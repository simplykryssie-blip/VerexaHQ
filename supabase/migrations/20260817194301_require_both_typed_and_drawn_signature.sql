-- The signature capture used to let a client choose either a typed name
-- OR a drawn signature. Now both are always required, on every
-- client-facing signing flow. Frontend already gates this (SignaturePad
-- shows both fields, submit stays disabled until both are filled), but
-- validating here too means a client can't sign with only one even if a
