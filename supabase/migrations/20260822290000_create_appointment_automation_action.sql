-- appointment.status_changed already exists as a trigger, but there was no
-- action to CREATE one -- appointments only ever came from the manual
-- staff-facing calendar UI. No slot-picking/availability algorithm exists
-- yet, so this is scoped to what's realistic without one: a fixed/relative-
-- time appointment request (N days out, at a configured time-of-day,
-- computed in the workspace's own timezone) landing with status
-- 'scheduled' for staff to confirm or reschedule via the existing
-- calendar UI -- not full self-service booking.
...
alter table public.automation_steps add constraint automation_steps_action_type_check
  check (action_type = ANY (ARRAY[
    ...
    'end_workflow', 'invite_to_portal', 'condition', 'create_appointment'
  ]));
