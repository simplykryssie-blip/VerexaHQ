-- Automations reconciliation, Phase 11/17: side-effect idempotency.
--
-- send_email/send_sms are already correctly deduplicated (notification_queue's
-- unique constraint on (workspace_id, template_key, dedupe_key), keyed on
-- 'automation_step:'||step_id||':'||run_id). Every other action with a real
-- side effect -- create_task, create_appointment, create_quote, add_note,
-- send_portal_message, send_document_for_signature -- has none: if
-- execute_automation_step is ever called twice for the same (run_id,
-- step_id) -- a retry after partial failure (task #130), a race the
-- concurrency fix in 20261101040000 closes for the cron path but not for
-- every possible caller, an operator manually re-invoking a step -- each of
-- these creates a second task, appointment, quote, note, portal message, or
-- signature request.
--
-- Same fix, applied uniformly: one nullable automation_dedupe_key column per
-- table, a partial unique index on it (mirroring notification_queue's own
-- pattern), and each insert sets it to 'automation_step:'||step_id||':'||
-- run_id -- unique per (step, run), so the SAME step firing again within the
-- SAME run's execution produces the SAME dedupe key and the insert becomes a
-- safe no-op that returns the original row's id (via `on conflict ... do
-- update set automation_dedupe_key = excluded.automation_dedupe_key
-- returning id`, a no-op update that still makes RETURNING give back the
-- existing row rather than nothing). A different run (a later, independent
-- firing of the same automation) gets a different run_id and therefore a
-- different key, so it is unaffected -- this is per-execution idempotency,
-- not "this step can only ever run once ever".
--
-- send_portal_message dedupes on the messages table (the actual side
-- effect), not message_threads (which is a legitimate find-or-create lookup
-- independent of any one automation step).
...
-- Full CREATE OR REPLACE against 20261101030000's known-correct body (this
-- migration's own authored baseline, not a stale pre-session snapshot),
-- with only the 6 action branches above changed.
  ...
  begin
    if v_step.action_type = 'delay' then
      ...
      if v_new_task_id is null then
        -- Already created by an earlier attempt at this same (step, run) --
        -- reuse its id for the trigger_snapshot bookkeeping below, but don't
        -- redo the business-hours due-date calc (it's relative to now() and
        -- would push the due date later on every retry).
      ...
      if v_run.is_test then
        ...
        if v_signature_request_id is null then
          -- Already sent by an earlier attempt at this same (step, run) --
          -- do not create a second signer/notification/audit-trail entry.
