-- Partner purchase entry point -- trigger + executor wiring.
--
-- Extends the three existing partner-lifecycle trigger functions and the
-- automation step executor to recognize a partner_prospect_id alongside
-- the existing connection_id, so a purchase with no Verexa workspace can
-- drive the same existing automation engine a connection-based purchase
-- already does. No new trigger infrastructure -- same tables, same
-- functions, same evaluate_automation_conditions/start_next_automation_step
-- call sites, branching internally on which buyer identity is present.

-- --------------------------------------------------------------------
-- Bug fix found while extending this path: _evaluate_condition_list
-- resolved v_onboarding via an inner join through firm_connections,
-- so any partner_onboarding.* condition silently evaluated against an
-- all-null record for a prospect-based onboarding (firm_connection_id
-- is null there). partner_onboardings.workspace_id is already the
-- correct, always-populated tenant-scoping column -- use it directly
-- instead of joining through the connection.
-- --------------------------------------------------------------------
...
-- --------------------------------------------------------------------
-- fire_firm_package_purchase_automations: branch the fired event name
-- on buyer identity (connection vs prospect), enrich context with the
-- purchase's external/payment fields, and pass partner_prospect_id
-- through to onboarding creation and the resulting automation run.
-- --------------------------------------------------------------------
...
begin
  ...
  if v_event in ('firm_package.purchased', 'partner_package.purchased') then
    perform public._get_or_create_partner_onboarding(new.parent_workspace_id, new.connection_id, new.package_id, new.id, new.partner_prospect_id);
  end if;
  ...
  loop
    if public.evaluate_automation_conditions(v_automation.conditions, v_context, new.parent_workspace_id, null, null, new.connection_id, null) then
      insert into public.automation_runs (workspace_id, automation_id, connection_id, partner_prospect_id, trigger_snapshot, status)
      values (new.parent_workspace_id, v_automation.id, new.connection_id, new.partner_prospect_id, v_context, 'running')
...
-- --------------------------------------------------------------------
-- fire_partner_onboarding_created_automations: resolve purchaser
-- identity from partner_prospects when firm_connection_id is null,
-- and thread partner_prospect_id through to the automation run.
-- --------------------------------------------------------------------
...
begin
  if new.firm_connection_id is not null then
    ...
    select p.first_name, p.last_name, p.email::text, p.phone into v_prospect
    from public.partner_prospects p
    where p.id = new.partner_prospect_id;
    v_buyer_name := nullif(btrim(coalesce(v_prospect.first_name, '') || ' ' || coalesce(v_prospect.last_name, '')), '');
  end if;
  ...
  v_context := jsonb_build_object(
    ...
    'purchaser_email', v_prospect.email,
    'purchaser_phone', v_prospect.phone
  );
...
-- --------------------------------------------------------------------
-- fire_partner_onboarding_status_changed_automations: same prospect
-- fallback for buyer_name, and partner_prospect_id threaded through.
-- --------------------------------------------------------------------
...
begin
  ...
  if new.firm_connection_id is not null then
    ...
    select p.first_name, p.last_name into v_prospect
    from public.partner_prospects p
    where p.id = new.partner_prospect_id;
    v_buyer_name := nullif(btrim(coalesce(v_prospect.first_name, '') || ' ' || coalesce(v_prospect.last_name, '')), '');
  end if;
...
-- --------------------------------------------------------------------
-- execute_automation_step: add a partner_prospect fallback (mirroring
-- the existing connection_id fallback) to send_email, send_sms,
-- create_task, add_tag, remove_tag, and assign_user(target: client);
-- move_pipeline_stage and change_stage degrade to a graceful skip
-- (v_skip_note, the same mechanism already used for test-mode/opt-out
-- skips) rather than raising, since pipeline stages are not extended
-- to a 4th polymorphic entity type in this change -- extending that
-- would touch ~7 separate pipeline functions and is out of scope here.
-- --------------------------------------------------------------------
    ...
    if v_step.action_type = 'delay' then
      ...
      if v_assign_target = 'client' then
        if v_run.client_id is not null then
          ...
          raise exception 'This workflow run has no client or partner prospect to assign';
        end if;
      else
        if v_run.engagement_id is null then
