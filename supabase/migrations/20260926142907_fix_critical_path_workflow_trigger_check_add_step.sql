-- Fixes a real bug surfaced by CI on 4 independent, unrelated PRs today:
-- run_critical_path_smoke_tests()'s workflow_trigger_check inserted its
-- fixture automation row (with status = 'published' set directly) BEFORE
-- authenticating as the workspace owner via set_config('request.jwt.claims',
-- ...) -- every other check in this same function authenticates first, this
-- one didn't. That was always a latent ordering bug, but it only started
-- failing once enforce_automation_publish_validation_trg (this same
-- reconciliation effort's own workflow-publish-validation-enforcement
-- migration, already applied to this staging project) began enforcing
-- has_permission(workspace_id, 'automations.manage') on publish -- with no
-- authenticated claims set yet, that check has nothing to authorize against
-- and raises "insufficient permissions to validate this automation".
--
-- Fix: authenticate as the owner before the insert, matching the pattern
-- every other block in this function already uses, and clear claims in the
-- exception handler too (the other blocks do this; this one didn't).
--
-- That surfaced a second, separate real gap once authentication was fixed:
-- validate_automation() also now correctly refuses to publish a step-less
-- automation ("This automation has no steps, so activating it does
-- nothing.") -- another pre-existing rule now enforced server-side by this
-- same reconciliation effort. The fixture automation had zero steps, which
-- was never exercised before either check exists. Adds one minimal
-- add_note step so the fixture is a realistic, valid published automation
-- (automation_steps cascades on automation delete, so no extra cleanup is
-- needed).
...
begin
  ...
  begin
    ...
    if v_service_id is null or v_client_id is null then
      ...
      -- Authenticate as the owner BEFORE inserting the automation: it's
      -- created with status = 'published' directly, and
      -- enforce_automation_publish_validation_trg's AFTER INSERT check
      -- needs an authenticated actor to authorize against, exactly like
      -- every other owner-privileged operation in this function already
      -- does.
      perform set_config('request.jwt.claims', json_build_object('sub', v_owner_id)::text, true);
