-- move_lead_stage and move_engagement_stage were never meant to be
-- fundamentally different capabilities -- the Workflows builder UI already
-- shows the identical "Pipeline" + "Target stage" pickers for both action
-- types (WorkflowBuilder.tsx). The only real asymmetry was in this
-- function: move_lead_stage auto-starts a lead_pipeline_run on the chosen
-- pipeline via start_lead_pipeline_run() if the lead isn't running one yet,
-- while move_engagement_stage just raised "This engagement has no active
-- pipeline to move" instead of doing the equivalent with
-- start_engagement_workflow() (which already exists and is already used by
-- create_engagement -- just never called from here). That's the gap this
-- closes: an engagement/client automation can now put an engagement onto
-- any chosen pipeline and advance it, the same way lead automations already
-- could -- no services.process_id configuration required.
