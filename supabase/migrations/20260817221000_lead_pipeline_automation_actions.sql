-- move_lead_stage now targets a real process_id/process_stage_id (like
-- move_engagement_stage) instead of the uncustomizable lead_stages list,
-- and change_stage ("Advance to the next pipeline stage") now also handles
-- a client-scoped run when there's no engagement -- both backed by
-- lead_pipeline_runs/lead_pipeline_stages from 20260817220000.
