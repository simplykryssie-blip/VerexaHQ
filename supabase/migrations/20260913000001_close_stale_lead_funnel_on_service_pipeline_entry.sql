-- A client landing in a real service pipeline (e.g. "Individual/Sched C
-- Onboarding") never closed out whatever lead-funnel pipeline run got them
-- there in the first place (e.g. "New Lead Onboarding & Intake") -- neither
-- move_pipeline_stage nor move_lead_to_service_pipeline ever looked at a
-- DIFFERENT pipeline's run, so the funnel run just sat "Active" forever on
-- its last-reached stage. This is not the same thing as the deliberate
-- concurrent-pipelines feature (a client with two genuinely simultaneous
-- services, e.g. Bookkeeping + Tax Resolution) -- those are two real
-- service pipelines and neither should touch the other. The distinction is
-- is_lead_funnel: a pipeline flagged as a funnel is meant to be passed
-- through once before landing in a real service pipeline, not run forever
-- alongside it.
alter table public.processes add column if not exists is_lead_funnel boolean not null default false;
comment on column public.processes.is_lead_funnel is
  'True for a pre-service intake/qualification pipeline a lead passes through once before landing in a service pipeline. When a client lands in a different (non-funnel) pipeline, any of their still-active funnel-flagged runs are automatically closed out -- see move_pipeline_stage / move_lead_to_service_pipeline in execute_automation_step.';
