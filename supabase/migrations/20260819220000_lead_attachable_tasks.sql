-- Tasks could only ever belong to an engagement. Every one of the MKB
-- workflows being rebuilt from GHL creates a task against a *lead* --
-- before any engagement exists (a follow-up task after a tag fires, a
-- no-show review task, an onboarding task triggered by a pipeline stage
-- change). The automation engine's create_task action hard-failed
-- (raise exception) whenever a workflow run had no engagement_id, which
-- is exactly that case. This makes engagement_id nullable, adds a
-- nullable client_id so a task can be attached directly to a lead/client
-- instead, and fixes every place that assumed engagement_id was always
-- present: the create_task automation action, and the three
-- fire_task_*_automations triggers (created/completed/overdue) that
-- derived the task's client_id by joining through the engagement.
alter table public.tasks alter column engagement_id drop not null;
...
alter table public.tasks add constraint tasks_engagement_or_client_chk
  check (engagement_id is not null or client_id is not null);
