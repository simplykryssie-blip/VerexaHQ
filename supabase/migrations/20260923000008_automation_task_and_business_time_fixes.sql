...
BEGIN
  ...
  v_old := $anchor$'first_name', v_eng.first_name,
    'firm_name', v_workspace.name,$anchor$;
  v_new := $replacement$'first_name', v_eng.first_name,
    'client_first_name', v_eng.first_name,
    'firm_name', v_workspace.name,$replacement$;

  IF position(v_new in v_def) = 0 THEN
    IF position(v_old in v_def) = 0 THEN
      v_def := replace(v_def, v_old, v_new);
    ELSE
      RAISE EXCEPTION 'client-name context anchor not found';
    END IF;
  END IF;

  v_old := $anchor$'created_tasks',
             coalesce(trigger_snapshot->'created_tasks', '{}'::jsonb) || jsonb_build_object(p_step_id::text, v_new_task_id)
           )$anchor$;
  v_new := $replacement$'created_tasks',
             coalesce(trigger_snapshot->'created_tasks', '{}'::jsonb) || jsonb_build_object(p_step_id::text, v_new_task_id),
  ...
  IF position(v_new in v_def) = 0 THEN
    IF position(v_old in v_def) = 0 THEN
      v_def := replace(v_def, v_old, v_new);
    ELSE
      RAISE EXCEPTION 'created-task snapshot anchor not found';
    END IF;
  END IF;

  v_old := $anchor$case when v_step.action_config ? 'due_in_days' then now() + make_interval(days => (v_step.action_config->>'due_in_days')::int) else null end,$anchor$;
  v_new := $replacement$case
