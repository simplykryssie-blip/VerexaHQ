-- Tasks gain a visibility label (internal, staff-only vs client, portal-visible),
-- so a workflow's create_task action (and manual task creation) can decide
-- whether a task should also show up on the client's own portal. Defaults to
-- internal so every existing task's behavior is unchanged.
alter table public.tasks
  add column visibility text not null default 'internal';

alter table public.tasks
  add constraint tasks_visibility_check check (visibility in ('internal', 'client'));

-- Portal read access: a client can see their own tasks that are explicitly
-- marked client-visible. tasks.client_id is only ever set for a client-level
-- task (no engagement yet); an engagement-level task is reached through
-- is_portal_accessible_entity_id, which already knows how to resolve an
-- engagement id back to its client -- same helper appointments/document
-- requests already use for portal SELECT policies.
create policy tasks_select_portal on public.tasks
  for select
  using (
    visibility = 'client'
    and (
      (client_id is not null and public.is_portal_user(client_id))
      or (engagement_id is not null and public.is_portal_accessible_entity_id(engagement_id))
    )
  );

-- Portal write access goes through this RPC rather than a direct RLS UPDATE
-- grant, matching the existing accept_quote/decline_quote pattern -- keeps
-- what a portal session can change to exactly status/completed_at on a task
-- that's actually theirs and actually client-visible.
...
grant execute on function public.set_client_task_completed(uuid, boolean) to authenticated;

-- Let a workflow's create_task step set visibility too, defaulting to
-- internal to match every existing automation's behavior unchanged.
