-- search_clients ran three separate ilike calls (first_name, last_name,
-- business_name), none of which matched the expression clients_name_trgm_idx
-- was actually built on (business_name || ' ' || first_name || ' ' ||
-- last_name), so the free-text search always fell back to a per-row
-- 3x-ilike sequential scan -- 5x slower than the unfiltered list even at 16
-- rows (measured: ~55.8ms vs ~11.2ms). Matching the indexed expression
-- exactly lets the planner use clients_name_trgm_idx once the table is
-- large enough for it to be worth it, and is already ~70x faster even in
-- seq-scan mode today (measured: 7.3ms -> 0.1ms), purely from evaluating
-- one concatenated ilike per row instead of three separate ones.
-- Found by the first Verexa Performance Agent run.
