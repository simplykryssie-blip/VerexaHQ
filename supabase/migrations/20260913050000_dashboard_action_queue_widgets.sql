-- Three new dashboard widget types for the "operational command center" pass:
-- 'unassigned_engagements' (open engagements nobody owns yet),
-- 'overdue_requests' (open document requests past their due date), and
-- 'failed_automations' (automation_runs that ended in status = 'failed').
-- All three are real, synchronously computed from existing tables -- same
-- pattern as stage_breakdown/deadline_risk before them, no new tables.
...
alter table public.dashboard_widgets
  ...
  check (widget_type = any (array[
    ...
    'payment_failures', 'top_services', 'engagement_pipeline', 'stage_breakdown', 'deadline_risk',
    'unassigned_engagements', 'overdue_requests', 'failed_automations'
  ]));
...
begin
  ...
  if v_widget_count = 0 then
    ...
    values
      ...
      (v_dashboard_id, 'unassigned_engagements', 'Unassigned Engagements', 15, '{}'::jsonb),
      (v_dashboard_id, 'overdue_requests', 'Overdue Client Requests', 16, '{}'::jsonb),
      (v_dashboard_id, 'failed_automations', 'Failed Automation Runs', 17, '{}'::jsonb);
  end if;
...
-- Backfill: every dashboard that predates this migration is missing the three
...
cross join (values
  ('unassigned_engagements', 'Unassigned Engagements', 1),
  ('overdue_requests', 'Overdue Client Requests', 2),
  ('failed_automations', 'Failed Automation Runs', 3)
) as w(widget_type, title, ord)
