-- compute_business_hours_deadline already does real weekly-hours math (per-
-- workspace hours, correct weekend skipping, verified live) but had no
-- concept of a one-off closure -- Christmas on a Wednesday still counted as
-- open. Reads the same system_settings row shape as business_hours/
-- booking_slot_minutes (key 'holidays', value a jsonb array of 'YYYY-MM-DD'
-- strings) so it fits the existing settings model instead of a new table.
...
begin
  ...
  while v_remaining > 0 and v_days_walked < 90 loop
    ...
    if v_holidays ? to_char(v_day_date, 'YYYY-MM-DD') then
      v_day := null;
    end if;
