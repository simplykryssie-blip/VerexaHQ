-- All of this session's test setup (Shanti, Octavionna, the "New Leads
-- Enter CRM" pipeline/organizer/automation trio, etc.) has been living in
-- the real "Verexa HQ CRM" workspace, cluttering production data. This
-- clones that workspace's business configuration -- pipelines, organizer
-- templates, services, email/SMS templates, automations, business hours --
-- into the empty "Demo - Independent PTIN" workspace so testing can move
-- there instead. Test *clients* (Shanti/Octavionna) are handled separately;
-- this migration only clones reusable configuration, not client records.
--
-- Every table here scopes its own uniqueness by workspace_id (or by a
-- parent row that itself belongs to one workspace), so names/slugs can be
-- copied verbatim -- only the handful of embedded foreign keys need
-- remapping: process_id/process_stage_id/automation_id inside
-- automation_steps.action_config, organizer_fields.parent_field_id
-- (self-referential), and the FK columns on services. template_slug values
-- and the "current_run" sentinel are resolved by string/convention at
-- runtime, not by id, so they're copied as-is.
do $$
declare
...
begin
  ...
  for r in select ofl.* from public.organizer_fields ofl join _org_tpl_map otm on otm.old_id = ofl.organizer_template_id order by ofl.display_order loop
    insert into public.organizer_fields (organizer_template_id, parent_field_id, field_type, label, help_text, display_order, is_required, options, conditional_logic, validation, body_html, client_profile_field, relationship_role)
    values (
      (select new_id from _org_tpl_map where old_id = r.organizer_template_id),
      null, r.field_type, r.label, r.help_text, r.display_order, r.is_required, r.options, r.conditional_logic, r.validation, r.body_html, r.client_profile_field, r.relationship_role
    )
    returning id into v_new_id;
  ...
  for r in select * from public.services where workspace_id = v_src loop
    insert into public.services (
      ...
      display_order, tags, status, created_by, document_folder_template_id, cloned_from_service_id
    )
    values (
      v_dst,
      ...
      (select new_id from _proc_map where old_id = r.process_id),
      (select new_id from _org_tpl_map where old_id = r.organizer_template_id),
      r.document_request_template_id,
      ...
      r.display_order, r.tags, r.status, null, r.document_folder_template_id, r.cloned_from_service_id
    );
  end loop;
  ...
  -- automation_steps -- remap process_id/process_stage_id/automation_id embedded
  -- in action_config; template_slug and the "current_run" sentinel are left
  -- untouched since they aren't id references.
  ...
  -- automation_step_edges -- a single set-based insert rather than a
  -- per-row loop. The automation canvas for these 3 automations was being
  -- live-edited via the UI while this migration was first drafted, and a
  -- row-by-row loop left an interleaving window that produced duplicate
  -- edges on the first attempt; one INSERT...SELECT closes that window.
