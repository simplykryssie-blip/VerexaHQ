-- Partner Automation Architecture Audit: closes the two concrete identity-
-- propagation gaps proven live to block partner-anchored (connection_id/
-- onboarding_id, no client_id/engagement_id) automation runs:
--
-- 1. assign_user (target=client) only ever wrote to clients.relationship_
--    manager_id or partner_prospects.assigned_staff_id, raising when neither
--    exists -- which is always true for a connection/onboarding-anchored
--    run. partner_onboardings already has its own assigned_staff_id column
--    (added by the completion engine) with no writer. Adds a third,
--    consistent fallback: when there's no client and no partner prospect but
--    the run carries onboarding_id, assign to that onboarding. Client and
--    engagement assignment paths are untouched.
--
-- 2. start_workflow's child-run insert only ever copied client_id/
--    engagement_id from the parent run, silently dropping connection_id/
--    onboarding_id/partner_prospect_id -- so any nested workflow started
--    from a partner-anchored parent produced a child run with no identity
--    at all, failing at its own first action. Copies all three alongside
--    the existing client_id/engagement_id copy; client/engagement workflows
--    are unaffected since those fields simply pass through null as before.
