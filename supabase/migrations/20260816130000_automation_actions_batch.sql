-- Adds 14 new automation action types (Lead, Client, Pipeline, Quote, Tag,
-- Note, Portal message, and workflow-chaining actions) plus implements the
-- two action types that were already reserved in the CHECK constraint but
-- fell through to "not yet supported" (assign_user, send_notification).
--
-- move_engagement_stage is deliberately forward-only: it advances the
-- engagement's active pipeline one stage at a time (marking each
-- intermediate stage 'Completed', which is exactly what the existing
-- advance_workflow_on_stage_completed trigger already does for manual
-- stage completion) until it reaches the configured target stage. Moving
-- an engagement backward through its pipeline is not supported by this
-- action -- that's a deliberately out-of-scope redesign of the pipeline
-- advance mechanism, not something this batch should touch.
--
-- start_workflow / end_workflow operate on automation_runs, not on
-- engagement pipelines -- "workflow" here refers to the automation itself
-- (this feature is literally named "Workflows" in the product). start_workflow
-- chains a second automation off the current run's context; end_workflow lets
-- an automation stop itself early. start_next_automation_step() already
-- no-ops when automation_runs.status <> 'running', so setting status to
-- 'completed' here is sufficient -- no other change needed.
...
alter table public.automation_steps add constraint automation_steps_action_type_check
  check (action_type = any (array[
    ...
    'send_organizer_template', 'create_engagement', 'send_engagement_letter',
    'send_document_request',
    'move_lead_stage', 'mark_lead_lost', 'convert_lead_to_client',
    'update_client', 'create_client', 'move_engagement_stage',
    'create_quote', 'send_quote', 'add_tag', 'remove_tag', 'add_note',
    'send_portal_message', 'start_workflow', 'end_workflow'
  ]));
