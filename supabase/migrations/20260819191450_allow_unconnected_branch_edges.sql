-- A condition step's branches (Yes/No/etc.) can now be defined -- label +
-- conditions -- before the user has dragged a connection to an actual next
-- step. to_step_id null means "this branch is defined but not wired to a
-- destination yet."
alter table public.automation_step_edges alter column to_step_id drop not null;

-- start_next_automation_step already treats a null v_next_step_id (after a
-- branch matches) as "nothing more to do, complete the run" via its
-- existing null check -- but that fell through to the same generic
-- completion as a normal end-of-chain. Give the "matched a branch with no
-- destination yet" case its own clear log message instead of looking
-- identical to a normal finish.
