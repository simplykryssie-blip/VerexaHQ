-- Source-control reconciliation only. This P1 operational-gate fix (closing
-- a workspace-operational bypass on the website/funnel section-reordering
-- RPCs and turn_on_service, found in a final sweep beyond the originally
-- scoped domain list) was applied live to production
-- (release-fix/p1-operational-gate-audit) but that branch was never merged
-- to main. Confirmed via direct comparison against the live function
-- definitions in project daxpavvsotvsyqqntddc that all four functions below
-- are BYTE-IDENTICAL to what is already running in production today. This
-- migration changes no live behavior -- it only makes the already-approved,
-- already-running fix reproducible from git (e.g. for a fresh environment
-- or disaster recovery), which previously would have silently lost this
-- security control.
