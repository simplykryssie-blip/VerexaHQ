-- Automations reconciliation, Phase 9/17: workflow-to-workflow recursion
-- guard + audit trail.
--
-- start_workflow inserts a fully independent new automation_runs row and
-- lets it run to completion on its own -- the parent run is unaffected and
-- both continue in parallel. This is intentional, existing, kept-as-is
-- behavior (confirmed this session by direct code read, and the product
-- decision this reconciliation pass makes is to preserve it, not change it).
-- What's missing is any bound on how many workflows can chain into each
-- other: start_next_automation_step's only loop guard is scoped to a single
-- automation's own step-edge graph and resets on every recursive call, so it
-- gives zero protection against automation A's start_workflow step starting
-- B, whose start_workflow step starts A again, forever (an existing
-- same-client duplicate-active-run trigger incidentally caps this around 2
-- hops for the same client/engagement, but does nothing for a fan-out across
-- different clients or a workspace with no such collision).
--
-- Fix: two new columns on automation_runs -- parent_run_id (the audit trail:
-- which run's start_workflow step created this one, null for a
-- trigger-started run) and chain_depth (0 for a trigger-started run,
-- parent's chain_depth + 1 for a workflow-started one). execute_automation_step's
-- start_workflow branch is patched to populate both and to refuse to start a
-- child once chain_depth would exceed a conservative platform-level maximum
-- (10) -- deliberately not a per-workspace or per-automation configurable
-- value, matching the explicit product requirement that this cap not be
-- something a customer can raise. Refusing is a normal validation failure on
-- this one step (recorded in automation_execution_logs, same as any other
-- start_workflow failure -- e.g. "Target automation is not available to
-- start"), not a hard crash of the parent run: the parent's own
-- exception-handling in execute_automation_step already turns any raised
-- exception here into a 'failed' log entry for that step and continues the
-- parent run's own status handling exactly as it does today for every other
-- start_workflow failure mode.
...
-- Full CREATE OR REPLACE against the exact body recovered in
-- 20261101000000_recover_automation_executor_live_state.sql (this
-- migration's own authored, known-correct baseline -- not a stale
-- pre-this-session snapshot), with only the start_workflow branch changed:
-- adds the depth check, and threads parent_run_id/chain_depth into the
-- child run insert.
