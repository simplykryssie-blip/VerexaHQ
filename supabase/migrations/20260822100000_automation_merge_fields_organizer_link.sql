-- Extends the automation engine's send_email/send_sms merge-field context.
-- Previously only engagement_number/client_name/firm_name/status were
-- available to any automation-triggered email or SMS -- not enough for the
-- new lead-intake automations, which need the client's first name, the tax
-- year, the firm's office phone/email, and (for organizer-related steps
-- specifically) a link to the right public organizer.
--
-- tax_year has no real data source yet -- no organizer question captures
-- it and no column tracks it pre-engagement -- so it's computed as last
-- calendar year, which is correct for the normal case (you're always
-- filing for the year that just ended). office_phone/office_email come
-- from branding.support_phone/support_email, the same fields the public
-- organizer page itself already shows to clients. organizer_link is
-- resolved per-step from action_config->>'organizer_template_id' (added
-- to a send_email/send_sms step's config), using branding.custom_domain
-- when a workspace has one, falling back to the main app domain.
