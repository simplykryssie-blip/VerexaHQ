-- create_client had no explicit check on p_client_type before the insert,
-- so a bad value (bypassing the app's own dropdown -- a direct RPC call, or
-- a future caller) surfaced as a raw Postgres CHECK constraint violation
-- ("new row for relation \"clients\" violates check constraint
-- \"clients_client_type_check\"") instead of a message a staff member could
-- act on. Found by the first Verexa QA Agent run.
