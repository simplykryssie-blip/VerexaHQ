-- Correction pass, blocker 3: execute_automation_step's operational gate
-- did a bare `return` with no discoverability marker, unlike
-- start_next_automation_step's identical gate (which sets
-- automation_runs.blocked_at). Under a narrow TOCTOU race -- workspace
-- goes non-operational between the cron's batch status fetch and this
-- specific row's RPC call -- a due, non-condition action step would hit
-- this gate, and the cron's unconditional post-RPC delete of the
-- automation_pending_steps row would then remove the only other record of
-- "waiting to resume." With blocked_at never set by this function, the
-- run became invisible to both the main pending-steps loop (no row left)
-- and the blocked-run resume phase in run-pending-automation-steps
-- (blocked_at is not null never matched) -- permanently stranded at
-- status='running' with no path back.
--
-- Fix: mirror start_next_automation_step's existing blocked_at/log-once
-- pattern here, verbatim. No other change to this function -- every
-- action branch, merge-field context, and error-handling path is
-- untouched. This alone closes the orphaning gap: whichever gate a given
-- invocation hits, the run is left in the same discoverable, resumable
-- blocked_at state, and the existing resume phase in
-- run-pending-automation-steps (added by 20261030050000) already knows
-- how to find and resume it once the workspace is active again -- no
-- cron changes needed.
