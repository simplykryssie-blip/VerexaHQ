-- Real gap found while checking the previous migration for side effects:
-- engagements.workflow_id is a denormalized column that two things rely on
-- being correct -- delete_workflow_pipeline() (20260816061500) uses it to
-- block deleting a pipeline that still has engagements actively running on
-- it, and the pipeline detail page uses it for a per-stage engagement
-- count. create_engagement already sets it at insert time, but
-- move_engagement_stage's new auto-start path (previous migration) only
-- created the workflow_runs row via start_engagement_workflow() -- it never
-- touched engagements.workflow_id. That meant an engagement attached to a
-- pipeline through this new automation path would be invisible to the
-- delete-safety-check: staff could delete a pipeline while engagements
-- attached via automation were still actively running on it, orphaning
-- their workflow_runs/workflow_stages rows. Fixed by also setting
-- engagements.workflow_id when this path starts a new workflow run.
