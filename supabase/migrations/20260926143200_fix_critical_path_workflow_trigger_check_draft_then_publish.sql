-- Third pass on the same workflow_trigger_check gap (see the two prior
-- migrations in this set): enforce_automation_publish_validation_trg fires
-- as an AFTER INSERT row-level trigger, so it evaluates step count at the
-- moment the automations row is inserted -- a later, separate `insert into
-- automation_steps` in the same function can never satisfy it, because the
-- validation already ran and failed before that second statement executes.
--
-- That's not a bug to work around -- it's the trigger doing exactly its
-- job. The real application never creates a new automation already
-- published; it creates a draft, lets the builder add steps, and only then
-- flips it to published, which is what fires the AFTER UPDATE branch of
-- the same trigger with steps already in place. The smoke test's one-shot
-- "insert already published" was a shortcut that predates this trigger and
-- was never compatible with it. Fix: make the fixture follow the same
-- draft -> add steps -> publish sequence the real app follows.
...
begin
  ...
  begin
    ...
    if v_service_id is null or v_client_id is null then
      ...
      -- Draft first (mirrors the real workflow builder): a fresh automation
      -- starts disabled/unpublished, so this insert never trips the
      -- publish-validation trigger's AFTER INSERT branch.
      ...
      -- Now publish, with the step already in place and claims already
      -- set -- this fires the trigger's AFTER UPDATE branch, exactly like
      -- a real publish from the workflow builder.
