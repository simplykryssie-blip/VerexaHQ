-- Re-reverts 20260827190000_signature_type_or_draw.sql: signing once again
-- requires BOTH a typed name AND a drawn signature on every client-facing
-- signing flow, restoring the exact behavior from
-- 20260817194301_require_both_typed_and_drawn_signature.sql. Frontend
-- (SignaturePad.tsx and its consumers) already shows both fields with no
-- toggle and gates submit on both being present; this restores the same
-- validation server-side so a request can't bypass the UI with only one.
