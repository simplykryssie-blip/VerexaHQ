-- Lets a "Send the document for signature" automation step also request a
-- signature from a linked contact of a given relationship type (e.g. the
-- client's spouse, or a business co-owner), not just the primary client.
-- The queued job carries the configured relationship type through to the
-- cron worker (app/api/cron/send-pending-engagement-letters/route.ts),
-- which resolves it against client_relationships at send time.
alter table public.pending_engagement_letter_sends
  add column additional_signer_relationship_type text;
