-- Fixes the original bug that started this whole thread: a firm_package.purchased/
-- .canceled automation run carries no client_id or engagement_id (there is no
-- client involved in a firm buying a package), but create_task/add_tag/
-- move_pipeline_stage all hard-required one and would raise the moment a real
-- purchase happened. Each of those three branches now falls back to the
-- connection_id already present in trigger_snapshot (populated by
-- fire_firm_package_purchase_automations) when there's no client/engagement.
-- Tags land on a new firm_connections.tags column (mirroring clients.tags,
-- since there's no client row to hold them); tasks use the firm_connection_id
-- column added in an earlier migration; pipeline moves teach start_pipeline_run/
-- advance_pipeline_stage/fire_pipeline_stage_entered_automations a third
-- entity_type alongside 'client'/'engagement'.

alter table public.firm_connections add column tags text[] not null default '{}';
...
-- Only 'client' and 'engagement' pipeline stage-entered automations exist as a
-- concept today (lead.stage_entered / engagement.stage_entered trigger types).
-- A firm_connection's pipeline stage can now genuinely reach 'In Progress'
-- (move_pipeline_stage supports it below), so this must recognize that case
-- and no-op rather than falling into the engagement branch, which would
-- misread a firm_connection id as an engagement id and fire corrupted
-- engagement.stage_entered automations.
...
begin
  ...
  if v_entity_type = 'client' then
    ...
    -- firm_connection (or any future entity type): no stage-entered automation
    -- concept exists for it yet -- safe no-op rather than a guess.
    return new;
  end if;
