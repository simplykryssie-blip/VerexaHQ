-- "Document signed?" originally checked only the single most recently sent
-- signature request in the run (trigger_snapshot->>'last_signature_request_id'),
-- which breaks the moment a workflow sends more than one document (e.g.
-- ERO onboarding's Partner Software & Bank Application AND a separate
-- Software License Agreement) -- there was no way to say which one a given
-- condition means. Switches to a per-step map, trigger_snapshot->
-- 'document_signatures' keyed by the sending step's id, and the condition's
-- value becomes "<step_id>|<true|false>" (same composite-value pattern as
-- client.organizer_status's "<template_id>|<status>"), so the workflow
-- builder can offer a dropdown of which "Send a document for signature"
-- step this condition is about.
  ...
  begin
    if v_step.action_type = 'delay' then
      ...
      if v_run.is_test then
        ...
        -- Keyed by this step's id (not a single global "last" pointer) so a
        -- workflow that sends more than one document keeps each one's
        -- status independently checkable.
        update public.automation_runs
