-- Completes the Stripe Package Purchase / Partner Onboarding architecture
-- whose database foundation (partner_prospects, firm_package_purchases'
-- external_* columns, workspace_partner_purchase_webhooks,
-- record_verified_partner_purchase, find_or_create_partner_prospect,
-- _get_or_create_partner_onboarding's 5-arg form) was already built and
-- applied in an earlier, uncommitted pass -- confirmed live via
-- pg_get_functiondef rather than assumed from any local file. Nothing in
-- that foundation is redesigned here. This migration adds the two pieces
-- still missing: (1) a package-side Stripe identifier so a completed
-- checkout can be deterministically mapped back to a specific Verexa
-- package, and (2) a package-side "purpose" so a purchase only creates a
-- partner_onboardings row when the package actually represents a
-- partner/service-bureau relationship -- today every package purchase
-- unconditionally called _get_or_create_partner_onboarding.

-- ---------------------------------------------------------------------
-- 1. Package <-> Stripe mapping
-- ---------------------------------------------------------------------
...
-- Defense in depth against an admin mapping the same external Stripe
-- object to two different packages in the same workspace by mistake --
-- scoped per-workspace (each workspace's Stripe identifiers are already
-- tenant-isolated by construction; this just catches operator error).
...
-- ---------------------------------------------------------------------
-- 2. Purpose-gate partner onboarding creation on package purchase
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 3. Two-step webhook credential setup: Verexa mints the endpoint/token
--    first (so the URL exists to paste into Stripe), the admin then
--    pastes back the signing secret STRIPE generates for that endpoint
--    when they add it in their own Stripe Dashboard -- there is no way to
--    supply your own secret to Stripe, so set_partner_purchase_webhook's
--    original "generate a random secret server-side" shape (from the
--    still-unreleased, never-wired-to-any-route prior pass) could never
--    have worked against a real Stripe webhook and is replaced before
...
-- _resolve_partner_purchase_webhook already returns null for signing_secret
-- when it's null (decrypt_firm_secret(null) = null) -- the webhook route
-- treats a null secret as "not configured yet" and rejects cleanly. No
-- change needed there.

-- ---------------------------------------------------------------------
-- 4. Nested-field condition resolution (Workstream B) -- the generic
--    condition fallback only ever did a flat top-level jsonb lookup
--    (p_context ->> v_field). partner_onboarding.created's trigger_snapshot
--    genuinely nests package_purchase as an object
--    (jsonb_build_object('package_purchase', jsonb_build_object('package_name', ...)))
--    while firm_package.purchased/canceled/partner_package.purchased's
--    snapshot stores the exact same field NAME as a flat literal key
--    containing a dot ('package_purchase.package_name'). Confirmed live
--    (pg_get_functiondef) that every other dotted condition field
--    (client.*, engagement.*, quote.*, task.*, document_request.*,
--    partner_onboarding.*, firm_connection.*) is resolved by its own
--    explicit case branch above this fallback, never by it -- so
--    package_purchase.package_name/.billing_cadence are the ONLY fields
--    that ever reach this fallback, confirmed via ConditionsEditor.tsx
--    (the only two condition options with no matching case branch).
--    Fix: try the existing flat lookup first (byte-for-byte unchanged
--    behavior for every already-working condition, including the flat
--    firm_package.purchased shape), and only when that misses AND the
--    field name contains a dot, fall back to real nested-path traversal.
-- ---------------------------------------------------------------------
...
-- Focused regression check for the fallback change above -- callable
-- directly (see tests/condition-evaluator-nested-field.test.ts) so the fix
-- has a real, running regression test rather than only this session's
-- interactive verification. Exercises exactly the two shapes in play: a
-- genuinely nested package_purchase object (the partner_onboarding.created
-- shape) and a flat literal-dotted-key context (the firm_package.purchased
-- shape) -- both must resolve to the same field name correctly, and a
-- non-dotted field and a condition with no match must behave unchanged.
