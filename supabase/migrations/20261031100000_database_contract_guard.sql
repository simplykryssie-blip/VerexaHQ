-- Migration history recovery -- this feature (this migration, lib/
-- databaseContractGuard.ts, tests/database-contract-guard.test.ts,
-- tests/fixtures/database-contract-baseline.json) was originally authored
-- as commit 25daf20 on the unmerged claude/verexa-remove-services-vaqbfx
-- branch and never brought to main. Already applied to the isolated
-- Supabase staging project (verexahq-test) used by
-- .github/workflows/critical-path-tests.yml -- deliberately NOT applied to
-- production, matching that same workflow's own scoping (it tests against
-- staging, never production). This file is a no-op replay against
-- staging, which already has it; a fresh environment built from these
-- migrations gets the same guard, and CI picks up the accompanying test
-- automatically now that it's committed.
--
