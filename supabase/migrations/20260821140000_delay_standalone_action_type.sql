-- Wait/Delay is now its own standalone step (ACTION_TYPES in
-- WorkflowBuilder.tsx), not a modifier field bolted onto every other
-- action -- staff pick "Wait / Delay" from the action list, set a
-- duration, and wire its connections on the diagram to control what it
-- waits before or after. The actual waiting is already fully handled by
-- start_next_automation_step's existing delay_minutes > 0 check (defers
-- to automation_pending_steps, picked up once due by the
-- run-pending-automation-steps cron) -- this just adds the missing no-op
-- handler so execute_automation_step doesn't reject action_type='delay'
-- once the deferred wait is over and it's time to just move on.
...
begin
  ...
  begin
    if v_step.action_type = 'delay' then
      null; -- the wait itself already happened via delay_minutes before this step was reached; nothing to execute.
