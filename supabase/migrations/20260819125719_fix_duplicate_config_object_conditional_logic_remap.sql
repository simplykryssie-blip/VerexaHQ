-- organizer_templates clones were carrying conditional_logic JSON that
-- referenced sibling organizer_fields by their OLD id, silently breaking
-- every conditional-logic rule on a cloned organizer. Remap field_id
-- references in conditional_logic to the newly-generated field ids using the
-- same tmp_field_map already built for the clone.
