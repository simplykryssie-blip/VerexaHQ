-- The "Create the engagement" automation action used to also start (or hand
-- off) a pipeline run for the new engagement whenever its service had a
-- default pipeline configured. That coupling is removed: creating an
-- engagement now only creates the engagement row. Putting it into a pipeline
-- is "Move to a pipeline stage" (move_pipeline_stage) -- its own separate
-- step, unchanged by this migration -- which staff add explicitly after
-- create_engagement when they want that.
