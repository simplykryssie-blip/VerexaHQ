-- New generic workflow action: "Send a document for signature". Unlike
-- send_engagement_letter (which requires an engagement, resolves a template
-- via the engagement's service, and renders a merge-filled PDF per
-- recipient), this sends a single document -- uploaded once, directly, when
-- the step is configured -- to whoever the automation run is actually about.
-- That's a client when one is attached (same as every other client-facing
-- action), or the connected firm on the other end of an ERO/PTIN onboarding
-- automation (trigger_snapshot->>'connection_id', e.g. firm_package.
-- purchased) when there's no client/engagement at all. Reuses the existing
-- signature_requests/signature_request_signers machinery and the public
-- /sign/[token] page as-is -- both are already generic (no dependency on
-- entity_type='engagement' or a client portal login).

alter table public.automation_steps drop constraint automation_steps_action_type_check;
alter table public.automation_steps add constraint automation_steps_action_type_check check (action_type = any (array[
  'send_email','send_sms','send_notification','create_task','assign_user','change_stage','request_approval','delay','webhook','escalate',
  'send_organizer_template','create_engagement','send_engagement_letter','send_document_request','move_pipeline_stage','mark_lead_lost',
  'convert_lead_to_client','update_client','create_client','create_quote','send_quote','add_tag','remove_tag','add_note','send_portal_message',
  'start_workflow','end_workflow','invite_to_portal','condition','create_appointment','add_dnd','remove_dnd','move_lead_to_service_pipeline',
  'business_hours_delay','send_document_for_signature'
]::text[]));
...
begin
  ...
  begin
    if v_step.action_type = 'delay' then
      ...
      if v_eng.primary_email is not null then
      ...
      elsif nullif(v_run.trigger_snapshot->>'connection_id', '') is not null then
        ...
        where fc.id = (v_run.trigger_snapshot->>'connection_id')::uuid;
      end if;
      ...
      if v_run.is_test then
        ...
        update public.automation_runs
        set trigger_snapshot = coalesce(trigger_snapshot, '{}'::jsonb) || jsonb_build_object('last_signature_request_id', v_signature_request_id)
        where id = p_run_id;
      end if;
...
-- run.document_signed: a live lookup by the run's own last_signature_request_id
-- (set above when a send_document_for_signature step runs), so a Condition
-- step re-checking this on every retry (see condition_step_retry_until_
-- matched) sees the real, current status rather than a value frozen at send
-- time. Generic by construction -- works for a client run or a connection-
-- context run identically, since it never touches client/engagement at all.
  ...
  loop
    ...
    if v_field = 'client.tags' then
      ...
      v_actual := case v_field
        ...
        when 'run.document_signed' then (coalesce(
          (select sr.status from public.signature_requests sr where sr.id = nullif(p_context->>'last_signature_request_id', '')::uuid),
          'not_sent'
        ) = 'completed')::text
