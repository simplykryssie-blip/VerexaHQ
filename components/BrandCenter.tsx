...
const EMPTY_CONTACT: FirmContact = {
  phone: "",
  email: "",
  website: "",
  address_line_1: "",
...
export default function BrandCenter() {
  ...
  const [contact, setContact] = useState<FirmContact>(EMPTY_CONTACT);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    const [brandRes, settingsRes, entitlementRes] = await Promise.all([
      supabase
        .from("workspace_brand_profiles")
        .select(
          "legal_name,dba_name,logo_url,alternate_logo_url,primary_color,secondary_color,accent_color,heading_font,body_font,logo_alignment,logo_width,document_footer,legal_disclaimer,watermark_text,show_powered_by",
        )
        .eq("workspace_id", activeWorkspaceId)
        .maybeSingle(),
      supabase
        .from("workspace_settings")
        .select("business_phone,business_email,business_website,business_address_line1,business_address_line2,business_city,business_state,business_postal_code")
        .eq("workspace_id", activeWorkspaceId)
        .maybeSingle(),
      supabase.rpc("can_remove_verexahq_branding", { p_workspace_id: activeWorkspaceId }),
    ]);

    if (brandRes.error) {
      setError(brandRes.error.message);
      return;
    }
    ...
    const s = settingsRes.data as Record<string, string | null> | null;
    setContact({
      phone: s?.business_phone ?? "",
      email: s?.business_email ?? "",
      website: s?.business_website ?? "",
      address_line_1: s?.business_address_line1 ?? "",
      address_line_2: s?.business_address_line2 ?? "",
      city: s?.business_city ?? "",
      state: s?.business_state ?? "",
      postal_code: s?.business_postal_code ?? "",
    });

    setCanRemoveBranding(entitlementRes.data === true);
    if (entitlementRes.data !== true && !next.show_powered_by) {
      // Plan no longer entitles hiding the mark -- never silently keep it hidden.
      setProfile((current) => ({ ...current, show_powered_by: true }));
    }
  ...
  const contactLine = useMemo(
    () => [contact.phone, contact.email, contact.website].filter(Boolean).join(" · "),
    ...
    <div className="space-y-6">
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_430px]">
        <section className="app-card p-5">
          ...
          <div className="mt-4 rounded-xl border border-line bg-paper p-4 text-sm">
            <div className="font-semibold text-ink">Firm contact information</div>
            <p className="mt-1 text-xs text-muted">
              {contactLine || address
                ? `${contactLine}${address ? (contactLine ? " · " : "") + address : ""}`
                : "Not added yet."}
            </p>
            <p className="mt-1 text-xs text-muted">
              This is managed in{" "}
              <Link href="/settings" className="font-semibold text-[#108A64] underline">
                Settings → Profile
              </Link>{" "}
              and used automatically on your documents and previews.
            </p>
          </div>
      ...
      <div className="sticky bottom-0 -mx-4 border-t border-line bg-paper/95 px-4 py-3 backdrop-blur sm:static sm:mx-0 sm:border-0 sm:bg-transparent sm:p-0">
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#108A64] px-4 py-3 text-sm font-semibold text-white hover:bg-[#0d7555] disabled:opacity-60 sm:w-auto"
        >
