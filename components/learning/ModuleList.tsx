...
import { BookOpen, CheckSquare, CheckCircle2, XCircle, Circle, Lock } from "lucide-react";
      ...
      {modules.map((m, i) => {
        ...
        const rowContent = (
          <>
            ...
            {unlocked && m.passed === true && <CheckCircle2 size={16} className="text-success" />}
            {unlocked && m.passed === false && <XCircle size={16} className="text-danger" />}
            {unlocked && m.passed == null && <Circle size={16} className="text-muted" />}
          </>
        );
